...
from PullentiPython.semantic.core.SemanticAbstractSlave import SemanticAbstractSlave
from PullentiPython.ner.core.VerbPhraseItemToken import VerbPhraseItemToken
...
from PullentiPython.semantic.core.SemanticLink import SemanticLink
...
from PullentiPython.ner.core.VerbPhraseToken import VerbPhraseToken
from PullentiPython.ner.core.NounPhraseToken import NounPhraseToken
from PullentiPython.semantic.utils.ControlModelQuestion import ControlModelQuestion
...
class SemanticHelper:
            ...
            if (npt1.noun.begin_token.get_morph_class_in_dictionary().is_personal_pronoun): 
                ...
                for r in res: 
                    if (r.question == ControlModelQuestion.get_base_genetive()): 
                        ...
                        break
                if (not has_gen): 
                    res.append(SemanticLink._new2643(True, npt1, sla2, 0.5, ControlModelQuestion.get_base_genetive()))
        if (onto is not None): 
            ...
            if (str2 is not None): 
                if (onto.check_link(str1, str2)): 
                    if (len(res) > 0): 
                        ...
                        res.append(SemanticLink._new2644(SemanticRole.STRONG, master, slave, 3))
        if (npt1 is not None): 
    ...
    def __try_create_inf(master : 'MetaToken', vpt2 : 'VerbPhraseToken', gr : 'DerivateGroup') -> typing.List['SemanticLink']:
        ...
        if (rol is not None): 
            res.append(SemanticLink._new2645((2 if rol != SemanticRole.COMMON else 1), ControlModelQuestion.get_to_do()))
        return res
    ...
    def __try_create_noun(npt1 : 'NounPhraseToken', slave : 'MetaToken', gr : 'DerivateGroup') -> typing.List['SemanticLink']:
        ...
        if (ok): 
            if (len(res) == 0): 
                res.append(SemanticLink._new2646(ControlModelQuestion.get_base_genetive(), SemanticRole.PACIENT, True))
            for r in res: 
                r.rank += (4)
                if (r.role == SemanticRole.COMMON): 
                    r.role = SemanticRole.STRONG
        ...
        if (prep is None and morph_.case_.is_nominative and not vpt1.first_verb.is_participle): 
            if (vpt1.first_verb.verb_morph is None): 
                ...
                if (ok): 
                    ...
                    addagent = False
                    if (cit00 is None): 
                        sl = SemanticLink._new2647(True, (SemanticRole.PACIENT if is_rev0 else SemanticRole.AGENT), 1, ControlModelQuestion.get_base_nominative(), is_rev0)
                    else: 
                        for kp in cit00.links.items(): 
                            q = kp[0]
                            if (q.check(None, MorphCase.NOMINATIVE)): 
                                sl = SemanticLink._new2648(kp[1], 2, q, is_rev0)
                    ...
                    if (sl is not None): 
                        ...
                        res.append(sl)
                        if (addagent): 
                            res.append(SemanticLink._new2649(SemanticRole.AGENT, sl.rank, sl.question))
        if (prep is None and is_rev1 and morph_.case_.is_instrumental): 
            no_instr = True
            cit00 = cit
            sl = None
            if (cit00 is None): 
                sl = SemanticLink._new2647(True, SemanticRole.AGENT, 1, ControlModelQuestion.get_base_instrumental(), True)
            else: 
                for kp in cit00.links.items(): 
                    q = kp[0]
                    if (q.check(None, MorphCase.INSTRUMENTAL)): 
                        sl = SemanticLink._new2649(kp[1], 2, q)
                        if (sl.role == SemanticRole.AGENT): 
                            sl.is_passive = True
                        break
            if (sl is not None): 
                ...
                if ((gr is not None and len(gr.model.items) > 0 and gr.model.items[0].typ == ControlModelItemType.VERB) and ControlModelQuestion.get_base_instrumental() in gr.model.items[0].links): 
                    sl.rank = (0)
                    sl0 = SemanticLink._new2652(sl.question, 1, gr.model.items[0].links[ControlModelQuestion.get_base_instrumental()])
                    res.insert(0, sl0)
        if (prep is None and morph_.case_.is_dative and ((cit is None or not ControlModelQuestion.get_base_dative() in cit.links))): 
            sl = SemanticLink._new2653(cit is None, SemanticRole.STRONG, 1, ControlModelQuestion.get_base_dative())
            ...
            if (ok): 
                if (len(res) == 0): 
                    ...
                    if (prep is None and is_rev1 and morph_.case_.is_nominative): 
                        ...
                        ok = True
                    if (ok): 
                        res.append(SemanticLink._new2654(SemanticRole.PACIENT, (ControlModelQuestion.get_base_nominative() if is_rev1 else ControlModelQuestion.get_base_accusative()), True))
                else: 
                    for r in res: 
                        r.rank += (4)
                        if (r.role == SemanticRole.COMMON): 
    ...
    def __create_roles(cit : 'ControlModelItem', prep : str, cas : 'MorphCase', res : typing.List['SemanticLink'], ignore_nomin_case : bool=False, ignore_instr_case : bool=False) -> None:
        ...
        if (roles is not None): 
            for kp in roles.items(): 
                sl = SemanticLink._new2649(kp[1], 2, kp[0])
