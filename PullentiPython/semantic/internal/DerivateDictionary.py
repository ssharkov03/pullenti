...
from PullentiPython.semantic.utils.DerivateGroup import DerivateGroup
from PullentiPython.morph.LanguageHelper import LanguageHelper
from PullentiPython.semantic.internal.ExplanTreeNode import ExplanTreeNode
...
class DerivateDictionary:
    ...
    def init(self, lang_ : 'MorphLang', lazy : bool) -> bool:
        ...
        names = Utils.getResourcesNames('PullentiPython.semantic.utils.properties', '.dat')
        for n in names: 
            if (Utils.endsWithString(n, rsname, True)): 
                inf = Utils.getResourceInfo('PullentiPython.semantic.utils.properties', n)
                if (inf is None): 
                    continue
                with Utils.getResourceStream('PullentiPython.semantic.utils.properties', n) as stream: 
                    stream.position = 0
                    self._m_all_groups.clear()
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos2656 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos2656)
                pos = wrappos2656.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos2660 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos2660)
            pos = wrappos2660.value
            while cou > 0: 
                wrappos2658 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos2658)
                pos = wrappos2658.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos2657 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos2657)
                    pos = wrappos2657.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos2659 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos2659)
            pos = wrappos2659.value
        self.__m_buf = wr
