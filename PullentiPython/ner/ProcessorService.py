...
from PullentiPython.ner.metadata.ImageWrapper import ImageWrapper
...
class ProcessorService:
    ...
    def initialize(lang : 'MorphLang'=None) -> None:
        ...
        from PullentiPython.ner.core.internal.BlockLine import BlockLine
        ...
        from PullentiPython.ner.core.ConjunctionHelper import ConjunctionHelper
    ...
    def create_processor() -> 'Processor':
        ...
        """
        from PullentiPython.ner.Processor import Processor
            ...
            if (img is not None): 
                for kp in img.items(): 
                    if (not kp[0] in ProcessorService.__m_images): 
                        ProcessorService.__m_images[kp[0]] = ImageWrapper._new2621(kp[0], kp[1])
    ...
    def get_image_by_id(image_id : str) -> 'ImageWrapper':
        ...
        if (image_id is not None): 
            res = None
            wrapres2622 = RefOutArgWrapper(None)
            inoutres2623 = Utils.tryGetValue(ProcessorService.__m_images, image_id, wrapres2622)
            res = wrapres2622.value
            if (inoutres2623): 
                return res
        if (ProcessorService.__m_unknown_image is None): 
            ProcessorService.__m_unknown_image = ImageWrapper._new2621("unknown", PullentiNerCoreInternalResourceHelper.get_bytes("unknown.png"))
    ...
    def add_image(image_id : str, content : bytearray) -> None:
        ...
        if (image_id is None): 
            return
        wr = ImageWrapper._new2621(image_id, content)
