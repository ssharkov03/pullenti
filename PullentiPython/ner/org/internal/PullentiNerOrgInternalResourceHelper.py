...
class PullentiNerOrgInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.ner.org.internal.properties', '.png;.dat')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.ner.org.internal.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.ner.org.internal.properties', n) as stream: 
