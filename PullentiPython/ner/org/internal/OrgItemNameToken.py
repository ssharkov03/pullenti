...
from PullentiPython.semantic.utils.DerivateService import DerivateService
from PullentiPython.ner.MetaToken import MetaToken
from PullentiPython.semantic.core.SemanticHelper import SemanticHelper
...
class OrgItemNameToken(MetaToken):
        ...
        if (t.is_value("ОРДЕНА", None) and t.next0_ is not None): 
            ...
            if (npt is not None): 
                ...
                if (((t1.is_value("ЗНАК", None) or t1.is_value("ДРУЖБА", None))) and (t1.whitespaces_after_count < 2)): 
                    ...
                    if (npt is not None): 
                        t1 = npt.end_token
                return OrgItemNameToken._new1519(t, t1, True)
            if (t.next0_.get_morph_class_in_dictionary().is_proper_surname): 
                return OrgItemNameToken._new1519(t, t.next0_, True)
            ppp = t.kit.process_referent("PERSON", t.next0_, None)
            if (ppp is not None): 
                return OrgItemNameToken._new1519(t, ppp.end_token, True)
            if ((t.whitespaces_after_count < 2) and BracketHelper.can_be_start_of_sequence(t.next0_, True, False)): 
                ...
                if (br is not None and (br.length_char < 40)): 
                    return OrgItemNameToken._new1519(t, br.end_token, True)
        ...
        if (res is None): 
            if (ext_onto): 
                if ((isinstance(t.get_referent(), GeoReferent)) or (((isinstance(t, TextToken)) and not t.is_char(';')))): 
                    return OrgItemNameToken._new1523(t, t, t.get_source_text())
            return None
    ...
    def __try_attach(t : 'Token', prev : 'OrgItemNameToken', ext_onto : bool) -> 'OrgItemNameToken':
        ...
        if (r is not None): 
            if (r.type_name == "DENOMINATION"): 
                return OrgItemNameToken._new1524(t, t, r.to_string_ex(True, t.kit.base_language, 0), True)
        ...
        if (tok is not None): 
            return OrgItemNameToken._new1525(t, tok.end_token, tok.termin.canonic_text, tok.termin.tag is None, tok.termin.tag is not None, tok.morph)
        tok = OrgItemNameToken.__m_std_names.try_parse(t, TerminParseAttr.NO)
        if ((tok) is not None): 
            return OrgItemNameToken._new1526(t, tok.end_token, tok.termin.canonic_text, True)
        eng = OrgItemEngItem.try_attach(t, False)
        if (eng is None and t.is_char(',')): 
            eng = OrgItemEngItem.try_attach(t.next0_, False)
        if (eng is not None): 
            return OrgItemNameToken._new1527(t, eng.end_token, eng.full_value, True)
        ...
        if (tt.is_char(',') and prev is not None): 
            ...
            if (ty is not None): 
                return None
            res = OrgItemNameToken._new1528(npt1.begin_token, npt1.end_token, npt1.morph, npt1.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
        ...
        if (((tt.is_char('&') or tt.is_value("AND", None) or tt.is_value("UND", None))) and prev is not None): 
            if ((isinstance(tt.next0_, TextToken)) and tt.length_char == 1 and tt.next0_.chars.is_latin_letter): 
                res = OrgItemNameToken._new1529(tt, tt.next0_, tt.next0_.chars)
                res.is_after_conjunction = True
        ...
        if (npt is not None and ((expl_ok or npt.morph.case_.is_genitive or ((prev is not None and not ((prev.morph.case_) & npt.morph.case_).is_undefined))))): 
            ...
            if (mc.is_noun and npt.chars.is_all_lower): 
                ...
                if ((not ca.is_dative and not ca.is_genitive and not ca.is_instrumental) and not ca.is_prepositional): 
                    return None
            res = OrgItemNameToken._new1528(npt.begin_token, npt.end_token, npt.morph, npt.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
            res.is_noun_phrase = True
            if ((npt.end_token.whitespaces_after_count < 2) and (isinstance(npt.end_token.next0_, TextToken))): 
                ...
                if (npt2 is not None and npt2.morph.case_.is_genitive and npt2.chars.is_all_lower): 
                    ...
                    if (mv2.is_adjective and mv2.is_verb): 
                        bi = MorphBaseInfo._new1531(npt.morph.case_, npt.morph.gender, npt.morph.number)
            ...
            if (npt is not None): 
                ...
                if (tt.term == "ПО"): 
                    ...
                    ok = npt.morph.case_.is_prepositional
                if (ok): 
                    res = OrgItemNameToken._new1533(t, npt.end_token, npt.morph, npt.get_normal_case_text(None, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False), npt.chars)
                    res.is_noun_phrase = True
            ...
            if (t1 is not None and t1.chars.is_latin_letter and not t1.chars.is_all_lower): 
                res = OrgItemNameToken._new1534(t, t1, t1.chars, t1.morph)
                ttt = t1.next0_
                first_pass2934 = True
                while True:
            ...
            if (tt.chars.is_cyrillic_letter): 
                ...
                if (not tt.is_whitespace_after and (tt.length_char < 5)): 
                    if (isinstance(tt.next0_, NumberToken)): 
                        return None
            res = OrgItemNameToken._new1535(tt, tt, tt.term, tt.morph)
            t = tt.next0_
            while t is not None: 
    ...
    def initialize() -> None:
        ...
        for s in ["ЗАКАЗЧИК", "ИСПОЛНИТЕЛЬ", "РАЗРАБОТЧИК", "БЕНЕФИЦИАР", "ПОЛУЧАТЕЛЬ", "ОТПРАВИТЕЛЬ", "ИЗГОТОВИТЕЛЬ", "ПРОИЗВОДИТЕЛЬ", "ПОСТАВЩИК", "АБОНЕНТ", "КЛИЕНТ", "ВКЛАДЧИК", "СУБЪЕКТ", "ПРОДАВЕЦ", "ПОКУПАТЕЛЬ", "АРЕНДОДАТЕЛЬ", "АРЕНДАТОР", "СУБАРЕНДАТОР", "НАЙМОДАТЕЛЬ", "НАНИМАТЕЛЬ", "АГЕНТ", "ПРИНЦИПАЛ", "ПРОДАВЕЦ", "ПОСТАВЩИК", "ПОДРЯДЧИК", "СУБПОДРЯДЧИК"]: 
            OrgItemNameToken.__m_std_tails.add(Termin._new264(s, s))
        for s in ["ЗАМОВНИК", "ВИКОНАВЕЦЬ", "РОЗРОБНИК", "БЕНЕФІЦІАР", "ОДЕРЖУВАЧ", "ВІДПРАВНИК", "ВИРОБНИК", "ВИРОБНИК", "ПОСТАЧАЛЬНИК", "АБОНЕНТ", "КЛІЄНТ", "ВКЛАДНИК", "СУБ'ЄКТ", "ПРОДАВЕЦЬ", "ПОКУПЕЦЬ", "ОРЕНДОДАВЕЦЬ", "ОРЕНДАР", "СУБОРЕНДАР", "НАЙМОДАВЕЦЬ", "НАЙМАЧ", "АГЕНТ", "ПРИНЦИПАЛ", "ПРОДАВЕЦЬ", "ПОСТАЧАЛЬНИК", "ПІДРЯДНИК", "СУБПІДРЯДНИК"]: 
            OrgItemNameToken.__m_std_tails.add(Termin._new572(s, MorphLang.UA, s))
        ...
        for k in range(2):
            ...
            for line0 in Utils.splitString(str0_, '\n', False): 
                ...
                if (k == 0): 
                    ...
                    OrgItemNameToken.__m_std_nouns.add(Termin._new1238(line, MorphLang.UA))
    
    @staticmethod
    def _new1519(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.is_ignored_part = _arg3
        return res
    
    @staticmethod
    def _new1523(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
        return res
    
    @staticmethod
    def _new1524(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
    ...
    @staticmethod
    def _new1525(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : bool, _arg5 : bool, _arg6 : 'MorphCollection') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
    ...
    @staticmethod
    def _new1526(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
    ...
    @staticmethod
    def _new1527(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
        res.is_std_tail = _arg4
        return res
    ...
    @staticmethod
    def _new1528(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : str) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.morph = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new1529(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'CharsInfo') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.chars = _arg3
        return res
    
    @staticmethod
    def _new1533(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : str, _arg5 : 'CharsInfo') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.morph = _arg3
        res.value = _arg4
        res.chars = _arg5
    ...
    @staticmethod
    def _new1534(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'CharsInfo', _arg4 : 'MorphCollection') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.chars = _arg3
        res.morph = _arg4
        return res
    ...
    @staticmethod
    def _new1535(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'MorphCollection') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
        res.morph = _arg4
        return res
    ...
    @staticmethod
    def _new2111(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.is_std_name = _arg3
        return res
    
    @staticmethod
    def _new2113(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo') -> 'OrgItemNameToken':
        res = OrgItemNameToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        return res
