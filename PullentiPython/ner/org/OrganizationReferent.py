...
class OrganizationReferent(Referent):
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            i = 0
            wrapi2153 = RefOutArgWrapper(0)
            inoutres2154 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi2153)
            i = wrapi2153.value
            if (inoutres2154): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres2155 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres2155 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        num = 0
        wrapnum2156 = RefOutArgWrapper(0)
        s = self.__correct_name(name, wrapnum2156)
        num = wrapnum2156.value
        if (s is None): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and Utils.compareStrings(typ.name, typ.typ, True) != 0 and ((len(typ.name) > len(typ.typ) or self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                num = 0
                wrapnum2157 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, wrapnum2157)
                num = wrapnum2157.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum2158 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, wrapnum2158)
                    num = wrapnum2158.value
                    if (ss is not None): 
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == ReferentsEqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    b = False
                    wrapb2160 = RefOutArgWrapper(False)
                    inoutres2161 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb2160)
                    b = wrapb2160.value
                    if (not inoutres2161): 
                        continue
