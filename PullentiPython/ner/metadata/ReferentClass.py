...
from PullentiPython.ner.metadata.Feature import Feature
...
class ReferentClass:
    ...
    def add_feature(self, attr_name : str, attr_caption : str, low_bound : int=0, up_bound : int=0) -> 'Feature':
        ...
        """
        res = Feature._new1458(attr_name, attr_caption, low_bound, up_bound)
    ...
    def find_feature(self, name_ : str) -> 'Feature':
        ...
        """
        ind = 0
        wrapind1459 = RefOutArgWrapper(0)
        inoutres1460 = Utils.tryGetValue(self.__m_hash, name_, wrapind1459)
        ind = wrapind1459.value
        if (not inoutres1460): 
            return None
        else: 
            return self.__m_features[ind]
