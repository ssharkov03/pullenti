...
from PullentiPython.ner.measure.internal.UnitsFactors import UnitsFactors
...
class UnitToken(MetaToken):
    ...
    def try_parse(t : 'Token', add_units : 'TerminCollection', prev : 'UnitToken', parse_unknown_units : bool=False) -> 'UnitToken':
        ...
        if (toks is not None): 
                ...
                if (toks[0].begin_token == toks[0].end_token or ((toks[0].end_token.is_char('.') and toks[0].begin_token.next0_ == toks[0].end_token))): 
                    if (toks[0].begin_token.is_value("М", None) or toks[0].begin_token.is_value("M", None)): 
                        if (prev is not None and prev.unit is not None and prev.unit.kind == MeasureKind.LENGTH): 
                            res = UnitToken._new1318(t0, toks[0].end_token, UnitsHelper.UMINUTE)
                            res.pow0_ = pow0__
                            if (is_neg): 
                                res.pow0_ = (- pow0__)
                            res.__correct()
                    ...
                    if (toks[0].begin_token.is_value("S", None) or toks[0].begin_token.is_value("С", None)): 
                        if (prev is not None and prev.unit is not None and ((prev.unit.kind == MeasureKind.LENGTH or is_neg))): 
                            res = UnitToken._new1318(t0, toks[0].end_token, UnitsHelper.USEC)
                            res.pow0_ = pow0__
                            if (is_neg): 
                                res.pow0_ = (- pow0__)
                            res.__correct()
            ...
            uts = list()
            for tok in toks: 
                res = UnitToken._new1318(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, Unit))
                res.pow0_ = pow0__
                if (is_neg): 
                    res.pow0_ = (- pow0__)
        ...
        if (t.is_char_of("º°")): 
            ...
            t1 = t.next0_.next0_
        if (t1 is not None): 
            res = UnitToken._new1318(t0, t1, UnitsHelper.UGRADUS)
            res.__check_doubt()
            t = t1.next0_
        ...
        if ((isinstance(t, TextToken)) and ((t.is_value("ОС", None) or t.is_value("OC", None)))): 
            ...
            if (str0_ == "оС" or str0_ == "oC"): 
                res = UnitToken._new1449(t, t, UnitsHelper.UGRADUSC, False)
                return res
        if (t.is_char('%')): 
            tt1 = t.next0_
            ...
            if ((isinstance(tt1, TextToken)) and ((tt1.is_value("ОБ", None) or tt1.is_value("ОБОРОТ", None)))): 
                re = UnitToken._new1318(t, tt1, UnitsHelper.UALCO)
                ...
                if (re.end_token.next0_ is not None and re.end_token.next0_.is_char(')') and t.next0_.is_char('(')): 
                    re.end_token = re.end_token.next0_
                return re
            return UnitToken._new1318(t, t, UnitsHelper.UPERCENT)
        if (add_units is not None): 
            tok = add_units.try_parse(t, TerminParseAttr.NO)
            if (tok is not None): 
                res = UnitToken._new1452(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, UnitReferent))
        ...
        if (mc.is_undefined): 
            ...
            return None
        res1 = UnitToken._new1453(t0, t1, pow0__, True)
        if (is_neg): 
            res1.pow0_ = (- pow0__)
    ...
    @staticmethod
    def _new1318(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        return res
    
    @staticmethod
    def _new1449(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit', _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        res.is_doubt = _arg4
        return res
    ...
    @staticmethod
    def _new1452(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'UnitReferent') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.ext_onto = _arg3
        return res
    
    @staticmethod
    def _new1453(_arg1 : 'Token', _arg2 : 'Token', _arg3 : int, _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.pow0_ = _arg3
        res.is_doubt = _arg4
        return res