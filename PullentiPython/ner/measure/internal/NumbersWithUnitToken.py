...
from PullentiPython.ner.measure.internal.Unit import Unit
...
from PullentiPython.ner.measure.internal.UnitsHelper import UnitsHelper
...
from PullentiPython.ner.measure.internal.MeasureHelper import MeasureHelper
...
class NumbersWithUnitToken(MetaToken):
    ...
    def try_parse_multi(t : 'Token', add_units : 'TerminCollection', attrs : 'NumberWithUnitParseAttr'=NumberWithUnitParseAttr.NO) -> typing.List['NumbersWithUnitToken']:
        ...
        if ((not mt.is_whitespace_after and MeasureHelper.is_mult_char_end(mt.end_token.next0_) and (isinstance(mt.end_token.next0_.next0_, NumberToken))) and len(mt.units) == 0): 
            ...
            if (terms is not None and len(terms) > 0): 
                mt.units.append(UnitToken._new1318(mt.end_token.next0_, mt.end_token.next0_, Utils.asObjectOrNull(terms[0].tag, Unit)))
    ...
    def _try_parse(t : 'Token', add_units : 'TerminCollection', attrs : 'NumberWithUnitParseAttr') -> 'NumbersWithUnitToken':
        ...
        is_diap_keyw = False
        min_max = 0
        wrapmin_max1325 = RefOutArgWrapper(min_max)
        ttt = NumbersWithUnitToken._is_min_or_max(t, wrapmin_max1325)
        min_max = wrapmin_max1325.value
        if (ttt is not None): 
            t = ttt.next0_
            if (t is None): 
                return None
        ...
        if (num is None): 
            ...
            if (tt is not None): 
                ...
                for u in UnitsHelper.UNITS: 
                    if (u.fullname_cyr == unam): 
                        uni = list()
                        uni.append(UnitToken._new1318(t, t, u))
                        break
                if (uni is None): 
                    return None
                res = NumbersWithUnitToken._new1320(t0, tt.end_token, about_)
                ...
                if ((uni is not None and len(uni) == 1 and uni[0].begin_token == uni[0].end_token) and uni[0].length_char > 3): 
                    rval = (1)
                    res = NumbersWithUnitToken._new1320(t0, uni[len(uni) - 1].end_token, about_)
                    t = res.end_token.next0_
                elif (has_keyw and (((attrs) & (NumberWithUnitParseAttr.CANBENON))) != (NumberWithUnitParseAttr.NO)): 
                    rval = math.nan
                    res = NumbersWithUnitToken._new1320(t0, t0, about_)
                    if (t is not None): 
            ...
            res = NumbersWithUnitToken._new1320(t0, num.end_token, about_)
            rval = num.real_value
        ...
        if (not second): 
            iii = 0
            wrapiii1324 = RefOutArgWrapper(iii)
            ttt = NumbersWithUnitToken._is_min_or_max(t, wrapiii1324)
            iii = wrapiii1324.value
    ...
    def _try_parsewhl(t : 'Token') -> 'MetaToken':
        ...
        if (len(txt) == 5 and ((txt[1] == 'Х' or txt[1] == 'X')) and ((txt[3] == 'Х' or txt[3] == 'X'))): 
            ...
            for i in range(3):
                ...
                if (ch == 'Г'): 
                    ...
                    return None
            return MetaToken._new1028(t, t, nams)
        t0 = t
        t1 = t
        while t is not None: 
        ...
        if (nams is None or (len(nams) < 2)): 
            return None
        return MetaToken._new1028(t0, t1, nams)
    ...
    def _initialize() -> None:
        ...
        t = Termin._new264("МЕНЕЕ", NumbersWithUnitToken.DiapTyp.LS)
        for s in lss: 
            t.add_variant(s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("НЕ МЕНЕЕ", NumbersWithUnitToken.DiapTyp.GE)
        for s in lss: 
            t.add_variant("НЕ " + s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("БОЛЕЕ", NumbersWithUnitToken.DiapTyp.GT)
        for s in gts: 
            t.add_variant(s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("НЕ БОЛЕЕ", NumbersWithUnitToken.DiapTyp.LE)
        for s in gts: 
            t.add_variant("НЕ " + s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("ОТ", NumbersWithUnitToken.DiapTyp.FROM)
        t.add_variant("С", False)
        t.add_variant("C", False)
        t.add_variant("НАЧИНАЯ С", False)
        t.add_variant("НАЧИНАЯ ОТ", False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("ДО", NumbersWithUnitToken.DiapTyp.TO)
        t.add_variant("ПО", False)
        t.add_variant("ЗАКАНЧИВАЯ", False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new264("НЕ ХУЖЕ", NumbersWithUnitToken.DiapTyp.UNDEFINED)
        NumbersWithUnitToken.M_TERMINS.add(t)
        NumbersWithUnitToken.M_SPEC = TerminCollection()
        t = Termin._new266("ПОЛЛИТРА", 0.5, "литр")
        t.add_variant("ПОЛУЛИТРА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new266("ПОЛКИЛО", 0.5, "килограмм")
        t.add_variant("ПОЛКИЛОГРАММА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new266("ПОЛМЕТРА", 0.5, "метр")
        t.add_variant("ПОЛУМЕТРА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new266("ПОЛТОННЫ", 0.5, "тонна")
        t.add_variant("ПОЛУТОННЫ", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        NumbersWithUnitToken.M_SPEC.add(t)
    
    @staticmethod
    def _new1311(_arg1 : 'Token', _arg2 : 'Token', _arg3 : float) -> 'NumbersWithUnitToken':
        res = NumbersWithUnitToken(_arg1, _arg2)
        res.single_val = _arg3
        return res
    
    @staticmethod
    def _new1320(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NumbersWithUnitToken':
        res = NumbersWithUnitToken(_arg1, _arg2)
        res.about = _arg3
        return res