...
from PullentiPython.ner.date.internal.DateItemToken import DateItemToken
...
from PullentiPython.ner.measure.MeasureKind import MeasureKind
from PullentiPython.ner.MetaToken import MetaToken
from PullentiPython.ner.measure.UnitReferent import UnitReferent
from PullentiPython.ner.core.MiscHelper import MiscHelper
from PullentiPython.ner.core.NounPhraseToken import NounPhraseToken
from PullentiPython.ner.measure.MeasureReferent import MeasureReferent
...
from PullentiPython.ner.measure.internal.UnitToken import UnitToken
...
class MeasureToken(MetaToken):
    ...
    def try_parse_minimal(t : 'Token', add_units : 'TerminCollection', can_omit_number : bool=False) -> 'MeasureToken':
        ...
        res = None
        if (len(mt) == 1): 
            res = MeasureToken._new1304(mt[0].begin_token, mt[len(mt) - 1].end_token, mt[0])
            res.__parse_internals(add_units)
            return res
        res = MeasureToken(mt[0].begin_token, mt[len(mt) - 1].end_token)
        for m in mt: 
            res.internals.append(MeasureToken._new1304(m.begin_token, m.end_token, m))
        return res
        ...
        if (self.end_token.next0_ is not None and ((self.end_token.next0_.is_char_of("\\/") or self.end_token.next0_.is_value("ПРИ", None)))): 
            ...
            if (mt1 is not None): 
                ...
                if (mt is not None and len(mt.units) > 0 and not UnitToken.can_be_equals(self.nums.units, mt.units)): 
                    self.internals.append(MeasureToken._new1304(mt.begin_token, mt.end_token, mt))
    ...
    def try_parse(t : 'Token', add_units : 'TerminCollection', can_be_set : bool=True, can_units_absent : bool=False, is_resctriction : bool=False, is_subval : bool=False) -> 'MeasureToken':
        ...
        whd = None
        minmax = 0
        wrapminmax1317 = RefOutArgWrapper(minmax)
        tt = NumbersWithUnitToken._is_min_or_max(t0, wrapminmax1317)
        minmax = wrapminmax1317.value
        if (tt is not None): 
            t = tt.next0_
        ...
        t1 = npt.end_token
        t = npt.end_token
        name_ = MetaToken._new696(npt.begin_token, npt.end_token, npt.morph)
        units = None
        units2 = None
        internals_ = list()
        not0_ = False
        ...
        while True:
            ...
            if (tt.is_table_control_char): 
                break
            wrapminmax1309 = RefOutArgWrapper(minmax)
            tt2 = NumbersWithUnitToken._is_min_or_max(tt, wrapminmax1309)
            minmax = wrapminmax1309.value
            if (tt2 is not None): 
                tt = tt2
                t = tt
                t1 = t
            ...
            if (tt.is_value("ПРИ", None)): 
                ...
                if (n1 is not None and len(n1.units) > 0): 
                    mt1 = MeasureToken._new1304(n1.begin_token, n1.end_token, n1)
                    internals_.append(mt1)
                    tt = mt1.end_token
                    t = tt
                    t1 = t
                ...
                if (t1.is_char(':')): 
                    ...
                    while True:
                        ...
                        if (ttt.is_char(';')): 
                            ...
                            break
                    if (len(li) > 1): 
                        res0 = MeasureToken._new1310(t0, li[len(li) - 1].end_token, li, True)
            ...
            if (units is not None and len(units) > 0): 
                if (t1 is None or t1.previous.is_char(':')): 
                    ...
                    if (t1 is None): 
                        ...
                        t1 = t1.previous
                    mts.append(NumbersWithUnitToken._new1311(t0, t1, math.nan))
            if (mts is None): 
                return None
        mt = mts[0]
        ...
        if (len(mts) > 1 and len(internals_) == 0): 
            if (len(mt.units) == 0): 
                if (units is not None): 
                    for m in mts: 
                        m.units = units
            res1 = MeasureToken._new1312(t0, mts[len(mts) - 1].end_token, name_.morph, True)
            res1.name = MiscHelper.get_text_value_of_meta_token(name_, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
            k = 0
            while k < len(mts): 
                ttt = MeasureToken._new1304(mts[k].begin_token, mts[k].end_token, mts[k])
                if (whd is not None): 
        ...
        if (len(mt.units) == 0): 
            ...
            if (units is None): 
                ...
                mt.units = units
        res = MeasureToken._new1314(t0, mt.end_token, name_.morph, internals_)
        ...
        if ((mts1 is not None and len(mts1) == 1 and (t1.whitespaces_before_count < 3)) and len(mts1[0].units) > 0 and not UnitToken.can_be_equals(mts[0].units, mts1[0].units)): 
            res.is_set = True
            res.nums = (None)
            res.internals.append(MeasureToken._new1304(mt.begin_token, mt.end_token, mt))
            res.internals.append(MeasureToken._new1304(mts1[0].begin_token, mts1[0].end_token, mts1[0]))
            res.end_token = mts1[0].end_token
        return res
    
    @staticmethod
    def _new1304(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'NumbersWithUnitToken') -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.nums = _arg3
        return res
    
    @staticmethod
    def _new1310(_arg1 : 'Token', _arg2 : 'Token', _arg3 : typing.List['MeasureToken'], _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.internals = _arg3
        res.is_empty = _arg4
        return res
    ...
    @staticmethod
    def _new1312(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.reliable = _arg4
        return res
    ...
    @staticmethod
    def _new1314(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : typing.List['MeasureToken']) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.internals = _arg4
        return res