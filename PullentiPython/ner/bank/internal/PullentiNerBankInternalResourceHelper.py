...
class PullentiNerBankInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.ner.bank.properties', '.png;.txt;.csv')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.ner.bank.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.ner.bank.properties', n) as stream: 
