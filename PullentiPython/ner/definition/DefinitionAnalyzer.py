...
from PullentiPython.ner.definition.internal.ParenthesisToken import ParenthesisToken
...
from PullentiPython.ner.definition.internal.MetaDefin import MetaDefin
...
from PullentiPython.ner.definition.internal.DefinitionAnalyzerEn import DefinitionAnalyzerEn
...
class DefinitionAnalyzer(Analyzer):
        ...
        if (kit.ontology is not None): 
            ...
            for it in kit.ontology.items: 
                if (isinstance(it.referent, DefinitionReferent)): 
                    ...
                    if (not termin in oh): 
                        oh[termin] = True
                        onto.add(Termin._new1027(termin, termin))
    ...
    def try_attach(t : 'Token', glos_regime : bool, onto : 'TerminCollection', max_char : int, this_is_def : bool=False) -> typing.List['ReferentToken']:
        ...
        if (name0 is None): 
            pass
        if (name0 is None): 
            dr.tag = (MetaToken._new1028(l0, l1, normal_left))
    ...
    def __try_attach_end(self, t : 'Token', onto : 'TerminCollection', max_char : int) -> typing.List['ReferentToken']:
        ...
        if ((((l1.end_char - l0.end_char)) * 2) > ((r1.end_char - r0.end_char))): 
            return None
        dr = DefinitionReferent._new1029(DefinitionKind.DEFINITION)
    ...
    def __try_attach_misc_token(t : 'Token') -> 'MetaToken':
        ...
        if (t.is_value("КАК", None)): 
            ...
            while True:
                ...
                break
            if (t1 is not None): 
                res = MetaToken._new1028(t, t1, MiscHelper.get_text_value(t, t1, GetTextAttr.KEEPQUOTES))
        ...
        if (npt is not None): 
            if (DefinitionAnalyzer.__m_misc_first_words.try_parse(npt.noun.begin_token, TerminParseAttr.NO) is not None): 
                res = MetaToken._new1028(t, npt.end_token, npt.get_normal_case_text(None, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False))
        ...
        if (t.is_value("В", None)): 
            ...
            if (npt is not None): 
                if (npt.noun.is_value("СМЫСЛ", None)): 
                    res = MetaToken._new1028(t, npt.end_token, MiscHelper.get_text_value(t, npt.end_token, GetTextAttr.NO))
                    res.morph.class0_ = MorphClass.NOUN
                    return res
        return None
