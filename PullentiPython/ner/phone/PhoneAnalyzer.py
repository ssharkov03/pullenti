...
from PullentiPython.ner.phone.internal.MetaPhone import MetaPhone
...
from PullentiPython.ner.phone.internal.PhoneItemToken import PhoneItemToken
    ...
    class PhoneAnalizerData(AnalyzerData):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            if (len(key) >= 10): 
                key = key[3:]
            ph_li = [ ]
            wrapph_li2425 = RefOutArgWrapper(None)
            inoutres2426 = Utils.tryGetValue(self.__m_phones_hash, key, wrapph_li2425)
            ph_li = wrapph_li2425.value
            if (not inoutres2426): 
                ph_li = list()
