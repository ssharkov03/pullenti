...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn2394 = RefOutArgWrapper(None)
                inoutres2395 = Utils.tryGetValue(tn.children, dig, wrapnn2394)
                nn = wrapnn2394.value
                if (not inoutres2395): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn2396 = RefOutArgWrapper(None)
            inoutres2397 = Utils.tryGetValue(nod.children, dig, wrapnn2396)
            nn = wrapnn2396.value
            if (not inoutres2397): 
                break
