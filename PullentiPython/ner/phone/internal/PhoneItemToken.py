...
from PullentiPython.ner.phone.PhoneKind import PhoneKind
...
from PullentiPython.ner.phone.internal.PhoneHelper import PhoneHelper
...
class PhoneItemToken(MetaToken):
    ...
    def __try_attach(t0 : 'Token') -> 'PhoneItemToken':
        ...
        if (isinstance(t0, NumberToken)): 
            ...
            if (t0.typ == NumberSpellingType.DIGIT and not t0.morph.class0_.is_adjective): 
                return PhoneItemToken._new2398(t0, t0, PhoneItemToken.PhoneItemType.NUMBER, t0.get_source_text())
            return None
        if (t0.is_char('.')): 
            return PhoneItemToken._new2398(t0, t0, PhoneItemToken.PhoneItemType.DELIM, ".")
        if (t0.is_hiphen): 
            return PhoneItemToken._new2398(t0, t0, PhoneItemToken.PhoneItemType.DELIM, "-")
        if (t0.is_char('+')): 
            if (not (isinstance(t0.next0_, NumberToken)) or t0.next0_.typ != NumberSpellingType.DIGIT): 
                ...
                if (i > 0): 
                    val = val[i:]
                return PhoneItemToken._new2398(t0, t0.next0_, PhoneItemToken.PhoneItemType.COUNTRYCODE, val)
        ...
        if (t0.is_char_of("(")): 
            if (isinstance(t0.next0_, NumberToken)): 
                ...
                if (et is None or val.tell() == 0): 
                    ...
                    return PhoneItemToken._new2403(t0, et, PhoneItemToken.PhoneItemType.CITYCODE, Utils.toStringStringIO(val), True)
            else: 
                ...
                if (tt1 is None or tt1.termin.tag is not None): 
                    ...
                    return PhoneItemToken._new2404(t0, tt1.end_token.next0_, PhoneItemToken.PhoneItemType.PREFIX, True, "")
                return None
        if ((t0.is_char('/') and (isinstance(t0.next0_, NumberToken)) and t0.next0_.next0_ is not None) and t0.next0_.next0_.is_char('/') and t0.next0_.length_char == 3): 
            return PhoneItemToken._new2403(t0, t0.next0_.next0_, PhoneItemToken.PhoneItemType.CITYCODE, str(t0.next0_.value), True)
        t1 = None
        ...
        if ((t0.is_value("Т", None) and t0.next0_ is not None and t0.next0_.is_char_of("\\/")) and t0.next0_.next0_ is not None and ((t0.next0_.next0_.is_value("Р", None) or t0.next0_.next0_.is_value("М", None)))): 
            ...
            t1 = tt.end_token
        res = PhoneItemToken._new2406(t0, t1, PhoneItemToken.PhoneItemType.PREFIX, "", ki)
        while True:
    ...
    def try_attach_additional(t0 : 'Token') -> 'PhoneItemToken':
        ...
        if (t.is_char(',')): 
            ...
            if (len(val0) >= 3 and (len(val0) < 7)): 
                return PhoneItemToken._new2398(t, t1, PhoneItemToken.PhoneItemType.ADDNUMBER, val0)
        br = False
        ...
        if (br): 
            if (t.next0_ is None or not t.next0_.is_char(')')): 
                return None
            t = t.next0_
        res = PhoneItemToken._new2398(t0, t, PhoneItemToken.PhoneItemType.ADDNUMBER, val)
        return res
    ...
    def try_attach_all(t0 : 'Token', max_count : int=15) -> typing.List['PhoneItemToken']:
        ...
        while True:
            ...
            if (p.item_type == PhoneItemToken.PhoneItemType.NUMBER and res[len(res) - 1].item_type == PhoneItemToken.PhoneItemType.NUMBER): 
                res.append(PhoneItemToken._new2398(t, t, PhoneItemToken.PhoneItemType.DELIM, " "))
            if (br): 
                p.is_in_brackets = True
            res.append(p)
            t = p.end_token
    ...
    def try_attach_alternate(t0 : 'Token', ph0 : 'PhoneReferent', pli : typing.List['PhoneItemToken']) -> 'PhoneItemToken':
            ...
            if (pli1 is not None and len(pli1) > 1): 
                ...
                if (len(pli1) <= len(pli)): 
                    ...
                    while ii < len(pli1): 
                        ...
                        ii += 1
                    if (ii >= len(pli1)): 
                        return PhoneItemToken._new2398(t0, pli1[len(pli1) - 1].end_token, PhoneItemToken.PhoneItemType.ALT, num)
            return PhoneItemToken._new2398(t0, t0.next0_, PhoneItemToken.PhoneItemType.ALT, t0.next0_.get_source_text())
        ...
        if ((t0.is_char('(') and (isinstance(t0.next0_, NumberToken)) and (t0.next0_.end_char - t0.next0_.begin_char) == 1) and t0.next0_.next0_ is not None and t0.next0_.next0_.is_char(')')): 
            return PhoneItemToken._new2398(t0, t0.next0_.next0_, PhoneItemToken.PhoneItemType.ALT, t0.next0_.get_source_text())
    ...
    def initialize() -> None:
        ...
        t = Termin._new2415("МОБИЛЬНЫЙ", MorphLang.RU, True, PhoneKind.MOBILE)
        t.add_abridge("МОБ.")
        t.add_abridge("Т.М.")
        t.add_abridge("М.Т.")
        t.add_abridge("М.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("СОТОВЫЙ", MorphLang.RU, True, PhoneKind.MOBILE)
        t.add_abridge("СОТ.")
        t.add_abridge("CELL.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("РАБОЧИЙ", MorphLang.RU, True, PhoneKind.WORK)
        t.add_abridge("РАБ.")
        t.add_abridge("Т.Р.")
        t.add_abridge("Р.Т.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        ...
        t.add_abridge("Г.Т.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("ДОМАШНИЙ", MorphLang.RU, True, PhoneKind.HOME)
        t.add_abridge("ДОМ.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        ...
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("ФАКС", MorphLang.RU, True, PhoneKind.FAX)
        t.add_abridge("Ф.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin("ЗВОНИТЬ", MorphLang.RU, True)
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("ПРИЕМНАЯ", MorphLang.RU, True, PhoneKind.WORK)
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin("PHONE", MorphLang.EN, True)
        t.add_abridge("PH.")
        t.add_variant("TELEFON", True)
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("DIRECT LINE", MorphLang.EN, True, PhoneKind.WORK)
        t.add_variant("DIRECT LINES", True)
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("MOBILE", MorphLang.EN, True, PhoneKind.MOBILE)
        t.add_abridge("MOB.")
        t.add_variant("MOBIL", True)
        t.add_abridge("M.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("FAX", MorphLang.EN, True, PhoneKind.FAX)
        t.add_abridge("F.")
        PhoneItemToken.M_PHONE_TERMINS.add(t)
        t = Termin._new2415("HOME", MorphLang.EN, True, PhoneKind.HOME)
        PhoneItemToken.M_PHONE_TERMINS.add(t)
    ...
    @staticmethod
    def _new2398(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PhoneItemType', _arg4 : str) -> 'PhoneItemToken':
    ...
    @staticmethod
    def _new2403(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PhoneItemType', _arg4 : str, _arg5 : bool) -> 'PhoneItemToken':
        res = PhoneItemToken(_arg1, _arg2)
        res.item_type = _arg3
        res.value = _arg4
    ...
    @staticmethod
    def _new2404(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PhoneItemType', _arg4 : bool, _arg5 : str) -> 'PhoneItemToken':
        res = PhoneItemToken(_arg1, _arg2)
        res.item_type = _arg3
    ...
    @staticmethod
    def _new2406(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PhoneItemType', _arg4 : str, _arg5 : 'PhoneKind') -> 'PhoneItemToken':
        res = PhoneItemToken(_arg1, _arg2)
        res.item_type = _arg3
        res.value = _arg4
        res.kind = _arg5
