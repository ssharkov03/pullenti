...
from PullentiPython.ner.business.internal.BusinessFactItemTyp import BusinessFactItemTyp
from PullentiPython.ner.TextToken import TextToken
from PullentiPython.ner.core.NounPhraseHelper import NounPhraseHelper
from PullentiPython.ner.business.BusinessFactKind import BusinessFactKind
...
class BusinessFactItem(MetaToken):
    ...
    def __try_parse(t : 'Token') -> 'BusinessFactItem':
        ...
        if (tok is not None): 
            ...
            if (ki != BusinessFactKind.UNDEFINED): 
                return BusinessFactItem._new497(t, tok.end_token, BusinessFactItemTyp.BASE, ki, tok.morph, tok.termin.tag2 is not None)
            ...
            while True:
                ...
                ki = (Utils.valToEnum(tok.termin.tag, BusinessFactKind))
                if (ki != BusinessFactKind.UNDEFINED): 
                    return BusinessFactItem._new498(t, tok.end_token, BusinessFactItemTyp.BASE, ki, tok.morph)
                tt = tok.end_token
        ...
        if (npt is not None): 
            if (((((npt.noun.is_value("АКЦИОНЕР", None) or npt.noun.is_value("ВЛАДЕЛЕЦ", None) or npt.noun.is_value("ВЛАДЕЛИЦА", None)) or npt.noun.is_value("СОВЛАДЕЛЕЦ", None) or npt.noun.is_value("СОВЛАДЕЛИЦА", None)) or npt.noun.is_value("АКЦІОНЕР", None) or npt.noun.is_value("ВЛАСНИК", None)) or npt.noun.is_value("ВЛАСНИЦЯ", None) or npt.noun.is_value("СПІВВЛАСНИК", None)) or npt.noun.is_value("СПІВВЛАСНИЦЯ", None)): 
                return BusinessFactItem._new498(t, npt.end_token, BusinessFactItemTyp.BASE, BusinessFactKind.HAVE, npt.morph)
        if (npt is not None): 
            if ((npt.noun.is_value("ОСНОВАТЕЛЬ", None) or npt.noun.is_value("ОСНОВАТЕЛЬНИЦА", None) or npt.noun.is_value("ЗАСНОВНИК", None)) or npt.noun.is_value("ЗАСНОВНИЦЯ", None)): 
                return BusinessFactItem._new498(t, npt.end_token, BusinessFactItemTyp.BASE, BusinessFactKind.CREATE, npt.morph)
        return None
    ...
    def initialize() -> None:
        ...
        for s in ["КУПИТЬ", "ПОКУПАТЬ", "ПРИОБРЕТАТЬ", "ПРИОБРЕСТИ", "ПОКУПКА", "ПРИОБРЕТЕНИЕ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.GET))
        for s in ["КУПИТИ", "КУПУВАТИ", "КУПУВАТИ", "ПРИДБАТИ", "ПОКУПКА", "ПРИДБАННЯ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.GET, MorphLang.UA))
        for s in ["ПРОДАТЬ", "ПРОДАВАТЬ", "ПРОДАЖА"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.SELL))
        for s in ["ПРОДАТИ", "ПРОДАВАТИ", "ПРОДАЖ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.SELL, MorphLang.UA))
        for s in ["ФИНАНСИРОВАТЬ", "СПОНСИРОВАТЬ", "ПРОФИНАНСИРОВАТЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.FINANCE))
        for s in ["ФІНАНСУВАТИ", "СПОНСОРУВАТИ", "ПРОФІНАНСУВАТИ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.FINANCE, MorphLang.UA))
        for s in ["ВЛАДЕТЬ", "РАСПОРЯЖАТЬСЯ", "КОНТРОЛИРОВАТЬ", "ПРИНАДЛЕЖАТЬ", "СТАТЬ ВЛАДЕЛЬЦЕМ", "КОНСОЛИДИРОВАТЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.HAVE))
        for s in ["ВОЛОДІТИ", "РОЗПОРЯДЖАТИСЯ", "КОНТРОЛЮВАТИ", "НАЛЕЖАТИ", "СТАТИ ВЛАСНИКОМ", "КОНСОЛІДУВАТИ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.HAVE, MorphLang.UA))
        for s in ["ПРИНАДЛЕЖАЩИЙ", "КОНТРОЛИРУЕМЫЙ", "ВЛАДЕЕМЫЙ", "ПЕРЕЙТИ ПОД КОНТРОЛЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new266(s, BusinessFactKind.HAVE, s))
        for s in ["НАЛЕЖНИЙ", "КОНТРОЛЬОВАНИЙ", "ВЛАДЕЕМЫЙ", "ПЕРЕЙТИ ПІД КОНТРОЛЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new510(s, BusinessFactKind.HAVE, s, MorphLang.UA))
        for s in ["ЗАКРЫТЬ СДЕЛКУ", "СОВЕРШИТЬ СДЕЛКУ", "ЗАВЕРШИТЬ СДЕЛКУ", "ЗАКЛЮЧИТЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.UNDEFINED))
        for s in ["ЗАКРИТИ ОПЕРАЦІЮ", "ЗДІЙСНИТИ ОПЕРАЦІЮ", "ЗАВЕРШИТИ ОПЕРАЦІЮ", "УКЛАСТИ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.UNDEFINED, MorphLang.UA))
        for s in ["ДОХОД", "ПРИБЫЛЬ", "ВЫРУЧКА"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.PROFIT))
        for s in ["ДОХІД", "ПРИБУТОК", "ВИРУЧКА"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.PROFIT, MorphLang.UA))
        for s in ["УБЫТОК"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.DAMAGES))
        for s in ["ЗБИТОК"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.DAMAGES, MorphLang.UA))
        for s in ["СОГЛАШЕНИЕ", "ДОГОВОР"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.AGREEMENT))
        for s in ["УГОДА", "ДОГОВІР"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.AGREEMENT, MorphLang.UA))
        for s in ["ИСК", "СУДЕБНЫЙ ИСК"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.LAWSUIT))
        for s in ["ПОЗОВ", "СУДОВИЙ ПОЗОВ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.LAWSUIT, MorphLang.UA))
        for s in ["ДОЧЕРНЕЕ ПРЕДПРИЯТИЕ", "ДОЧЕРНЕЕ ПОДРАЗДЕЛЕНИЕ", "ДОЧЕРНЯЯ КОМПАНИЯ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new264(s, BusinessFactKind.SUBSIDIARY))
        for s in ["ДОЧІРНЄ ПІДПРИЄМСТВО", "ДОЧІРНІЙ ПІДРОЗДІЛ", "ДОЧІРНЯ КОМПАНІЯ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new265(s, BusinessFactKind.SUBSIDIARY, MorphLang.UA))
    
    __m_base_onto = None
    
    @staticmethod
    def _new497(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BusinessFactItemTyp', _arg4 : 'BusinessFactKind', _arg5 : 'MorphCollection', _arg6 : bool) -> 'BusinessFactItem':
    ...
    @staticmethod
    def _new498(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BusinessFactItemTyp', _arg4 : 'BusinessFactKind', _arg5 : 'MorphCollection') -> 'BusinessFactItem':
        res = BusinessFactItem(_arg1, _arg2)
        res.typ = _arg3
        res.base_kind = _arg4
        res.morph = _arg5
