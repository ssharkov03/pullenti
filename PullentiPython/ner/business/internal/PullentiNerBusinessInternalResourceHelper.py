...
class PullentiNerBusinessInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.ner.business.properties', '.png;.txt;.jpg')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.ner.business.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.ner.business.properties', n) as stream: 
