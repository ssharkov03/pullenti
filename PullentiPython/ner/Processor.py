...
from PullentiPython.unisharp.Misc import CancelEventArgs
...
from PullentiPython.ner.core.internal.ProgressPeace import ProgressPeace
...
from PullentiPython.ner.AnalysisResult import AnalysisResult
...
class Processor(object):
    ...
    def find_analyzer(self, name : str) -> 'Analyzer':
        ...
        """
        a = None
        wrapa2613 = RefOutArgWrapper(None)
        inoutres2614 = Utils.tryGetValue(self.__m_analyzers_hash, Utils.ifNotNull(name, ""), wrapa2613)
        a = wrapa2613.value
        if (inoutres2614): 
            return a
    ...
    def process_next(self, ar : 'AnalysisResult') -> None:
        ...
        if (ar is None): 
            return
        kit = AnalysisKit._new2615(self, ar.ontology)
        kit._init_from(ar)
    ...
    def _process(self, text : 'SourceOfAnalysis', onto_regine : bool, no_log : bool, ext_ontology : 'ExtOntology'=None, lang : 'MorphLang'=None) -> 'AnalysisResult':
        ...
        kit = AnalysisKit._new2616(text, False, lang, self.__progress_changed_event_handler_on_progress_handler, ext_ontology, self, onto_regine)
        ar = AnalysisResult()
        sw0.stop()
        msg = None
    ...
    def __prepare_progress(self) -> None:
        with self.__m_progress_peaces_lock: 
            ...
            max0_ = co * 100
            max0_ /= (total)
            self.__m_progress_peaces[self] = ProgressPeace._new2617(0, max0_)
            for wf in self.analyzers: 
                if (not wf.ignore_this_analyzer): 
                    min0_ = max0_
                    ...
                    if (not wf in self.__m_progress_peaces): 
                        self.__m_progress_peaces[wf] = ProgressPeace._new2617(min0_, max0_)
    ...
    def _on_progress_handler(self, sender : object, e0_ : ProgressEventArgs) -> None:
        ...
        if (e0_.progressPercentage >= 0): 
            pi0_ = None
            with self.__m_progress_peaces_lock: 
                wrappi2619 = RefOutArgWrapper(None)
                inoutres2620 = Utils.tryGetValue(self.__m_progress_peaces, Utils.ifNotNull(sender, self), wrappi2619)
                pi0_ = wrappi2619.value
                if (inoutres2620): 
