...
from PullentiPython.ner.core.internal.TextsCompareType import TextsCompareType
...
class Referent:
    ...
    def add_slot(self, attr_name : str, attr_value : object, clear_old_value : bool, stat_count : int=0) -> 'Slot':
        ...
        Returns:
            Slot: слот(атрибут)
        """
        from PullentiPython.ner.Slot import Slot
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres2626 = RefOutArgWrapper(0)
        inoutres2627 = Utils.tryParseInt(str0_, wrapres2626)
        res = wrapres2626.value
        if (not inoutres2627): 
            return def_value
        return res
    ...
    def add_occurence_of_ref_tok(self, rt : 'ReferentToken') -> None:
        self.add_occurence(TextAnnotation._new934(rt.kit.sofa, rt.begin_char, rt.end_char, rt.referent))
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new2629(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new2630(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis', new_format : bool=False) -> None:
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new2631(sofa, self)
