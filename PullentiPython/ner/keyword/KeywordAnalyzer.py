...
from PullentiPython.ner.bank.BankDataReferent import BankDataReferent
from PullentiPython.ner.core.Termin import Termin
from PullentiPython.ner.phone.PhoneReferent import PhoneReferent
from PullentiPython.ner.keyword.internal.AutoannoSentToken import AutoannoSentToken
from PullentiPython.ner.denomination.DenominationReferent import DenominationReferent
from PullentiPython.ner.MetaToken import MetaToken
from PullentiPython.ner.keyword.internal.KeywordMeta import KeywordMeta
...
from PullentiPython.ner.denomination.DenominationAnalyzer import DenominationAnalyzer
...
class KeywordAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (npt is None): 
                ...
                if (mc.is_verb and not mc.is_preposition): 
                    ...
                    if (t.is_value("МОЧЬ", None) or t.is_value("WOULD", None)): 
                        continue
                    kref = KeywordReferent._new1287(KeywordType.PREDICATE)
                    ...
                    rt1 = ReferentToken._new956(ad.register_referent(kref), t, t, t.morph)
                    kit.embed_token(rt1)
                    t = (rt1)
                    continue
                continue
            ...
            while True:
                ...
                if (mc.is_misc): 
                    if (MiscHelper.is_eng_article(tt)): 
                        continue
                kref = KeywordReferent._new1287(KeywordType.OBJECT)
                norm = tt.lemma
                kref.add_slot(KeywordReferent.ATTR_VALUE, norm, False, 0)
                if (norm != "ЕСТЬ"): 
                ...
                kref = (Utils.asObjectOrNull(ad.register_referent(kref), KeywordReferent))
                KeywordAnalyzer.__set_rank(kref, cur, max0_)
                rt1 = ReferentToken._new956(kref, tt, tt, tt.morph)
                kit.embed_token(rt1)
            ...
            if (len(li) > 1): 
                ...
                kref = (Utils.asObjectOrNull(ad.register_referent(kref), KeywordReferent))
                KeywordAnalyzer.__set_rank(kref, cur, max0_)
                rt1 = ReferentToken._new956(kref, t0, t, npt.morph)
                kit.embed_token(rt1)
                t = (rt1)
        cur = 0
        t = kit.first_token
        ...
        while True:
            ...
            rt1 = ReferentToken._new956(kw_un, t, t1, t.morph)
            kit.embed_token(rt1)
            t = (rt1)
    ...
    def __add_referents(self, ad : 'AnalyzerData', t : 'Token', cur : int, max0_ : int) -> 'Token':
        ...
        if (isinstance(r, DenominationReferent)): 
            ...
            kref0 = KeywordReferent._new1287(KeywordType.REFERENT)
            for s in dr.slots: 
        ...
        if (isinstance(r, MoneyReferent)): 
            ...
            kref0 = KeywordReferent._new1287(KeywordType.OBJECT)
        ...
        while tt is not None and tt.end_char <= t.end_char: 
            ...
            tt = tt.next0_
        kref = KeywordReferent._new1287(KeywordType.REFERENT)
        norm = None
        if (r.type_name == "GEO"): 
