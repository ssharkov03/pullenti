...
from PullentiPython.ner.sentiment.SentimentKind import SentimentKind
from PullentiPython.ner.sentiment.SentimentReferent import SentimentReferent
from PullentiPython.ner.MetaToken import MetaToken
from PullentiPython.ner.sentiment.internal.MetaSentiment import MetaSentiment
from PullentiPython.ner.business.internal.PullentiNerBusinessInternalResourceHelper import PullentiNerBusinessInternalResourceHelper
...
class SentimentAnalyzer(Analyzer):
    ...
    def images(self) -> typing.List[tuple]:
        res = dict()
        res[MetaSentiment.IMAGE_ID] = PullentiNerBusinessInternalResourceHelper.get_bytes("neutral.png")
        res[MetaSentiment.IMAGE_ID_GOOD] = PullentiNerBusinessInternalResourceHelper.get_bytes("good.png")
        res[MetaSentiment.IMAGE_ID_BAD] = PullentiNerBusinessInternalResourceHelper.get_bytes("bad.png")
        return res
    ...
    def initialize() -> None:
        ...
        try: 
            for i in range(2):
                str0_ = PullentiNerBusinessInternalResourceHelper.get_string(("Positives.txt" if i == 0 else "Negatives.txt"))
                if (str0_ is None): 
                    continue
                for line0 in Utils.splitString(str0_, '\n', False): 
                    ...
                    SentimentAnalyzer.__m_termins.add(Termin._new264(line, coef))
        except Exception as ex: 
            pass
        for s in ["ОЧЕНЬ", "СИЛЬНО"]: 
            SentimentAnalyzer.__m_termins.add(Termin._new264(s, 0))
