...
from PullentiPython.ner.measure.internal.NumberWithUnitParseAttr import NumberWithUnitParseAttr
from PullentiPython.ner.core.Termin import Termin
from PullentiPython.ner.core.NumberExType import NumberExType
from PullentiPython.ner.date.DateRangeReferent import DateRangeReferent
...
from PullentiPython.ner.date.internal.DateTokenData import DateTokenData
from PullentiPython.ner.MetaToken import MetaToken
from PullentiPython.ner.Token import Token
from PullentiPython.ner.date.DatePointerType import DatePointerType
...
from PullentiPython.ner.date.DateAnalyzer import DateAnalyzer
from PullentiPython.ner.measure.internal.NumbersWithUnitToken import NumbersWithUnitToken
...
class DateItemToken(MetaToken):
    ...
    def __try_attach(t : 'Token', prev : typing.List['DateItemToken'], detail_regime : bool) -> 'DateItemToken':
        from PullentiPython.ner.measure.internal.MeasureToken import MeasureToken
        if (t is None): 
            return None
        ...
        if (flt != DateItemToken.FirstLastTyp.NO and t.next0_ is not None): 
            ...
            if (dty != DateItemToken.DateItemType.NUMBER): 
                res = DateItemToken._new837(t, t1, dty, flt, 1)
            ...
            if (t1.is_value("ГОД", "РІК") and NumberHelper.try_parse_roman(t1.next0_) is not None): 
                return DateItemToken._new838(t, t1, DateItemToken.DateItemType.POINTER, (DatePointerType.END if flt == DateItemToken.FirstLastTyp.LAST else DatePointerType.BEGIN))
        if (nt is not None): 
            ...
            if (NumberHelper.try_parse_age(nt) is not None): 
                return None
            tt = None
            res = DateItemToken._new839(begin, end, DateItemToken.DateItemType.NUMBER, nt.int_value, nt.morph)
        ...
        if ((txt[0] == 'I' or txt[0] == 'X' or txt[0] == 'Х') or txt[0] == 'V' or ((t0.chars.is_latin_letter and t0.chars.is_all_upper))): 
            ...
            if (lat is not None and lat.end_token.next0_ is not None and lat.int_value is not None): 
                ...
                if (tt.is_value("КВАРТАЛ", None) and val > 0 and val <= 4): 
                    return DateItemToken._new840(t, tt, DateItemToken.DateItemType.QUARTAL, val)
                if (tt.is_value("ПОЛУГОДИЕ", "ПІВРІЧЧЯ") and val > 0 and val <= 2): 
                    return DateItemToken._new840(t, lat.end_token.next0_, DateItemToken.DateItemType.HALFYEAR, val)
                if (tt.is_value("ВЕК", "ВІК") or tt.is_value("СТОЛЕТИЕ", "СТОЛІТТЯ")): 
                    return DateItemToken._new840(t, lat.end_token.next0_, DateItemToken.DateItemType.CENTURY, val)
                if (tt.is_value("ДЕСЯТИЛЕТИЕ", "ДЕСЯТИЛІТТЯ") or tt.is_value("ДЕКАДА", None)): 
                    return DateItemToken._new840(t, lat.end_token.next0_, DateItemToken.DateItemType.TENYEARS, val)
                if (((tt.is_value("В", None) or tt.is_value("ВВ", None))) and tt.next0_ is not None and tt.next0_.is_char('.')): 
                    if (prev is not None and len(prev) > 0 and prev[len(prev) - 1].typ == DateItemToken.DateItemType.POINTER): 
                        return DateItemToken._new840(t, tt.next0_, DateItemToken.DateItemType.CENTURY, val)
                ...
                if (tt.is_hiphen): 
                    ...
                    if (lat2 is not None and lat2.int_value is not None and lat2.end_token.next0_ is not None): 
                        if (lat2.end_token.next0_.is_value("ВЕК", "ВІК") or lat2.end_token.next0_.is_value("СТОЛЕТИЕ", "СТОЛІТТЯ")): 
                            ...
                            return DateItemToken._new846(t, lat.end_token, DateItemToken.DateItemType.CENTURY, val, ((ddd.new_age if ddd is not None else 0)))
                pr0 = None
                ttt = tt
                first_pass2840 = True
                while True:
                    ...
                    if (nex.typ == DateItemToken.DateItemType.CENTURY or nex.typ == DateItemToken.DateItemType.QUARTAL): 
                        return DateItemToken._new846(t, lat.end_token, nex.typ, val, nex.new_age)
                    break
        if (t is None): 
            return None
        if (t is not None and t.is_value("НАПРИКІНЦІ", None)): 
            return DateItemToken._new848(t, t, DateItemToken.DateItemType.POINTER, DatePointerType.END, "конец")
        if (t is not None and t.is_value("ДОНЕДАВНА", None)): 
            return DateItemToken._new848(t, t, DateItemToken.DateItemType.POINTER, DatePointerType.TODAY, "сегодня")
        if (prev is None): 
            if (t is not None): 
                if (t.is_value("ОКОЛО", "БІЛЯ") or t.is_value("ПРИМЕРНО", "ПРИБЛИЗНО") or t.is_value("ABOUT", None)): 
                    return DateItemToken._new848(t, t, DateItemToken.DateItemType.POINTER, DatePointerType.ABOUT, "около")
            if (t.is_value("ОК", None) or t.is_value("OK", None)): 
                if (t.next0_ is not None and t.next0_.is_char('.')): 
                    return DateItemToken._new848(t, t.next0_, DateItemToken.DateItemType.POINTER, DatePointerType.ABOUT, "около")
                return DateItemToken._new848(t, t, DateItemToken.DateItemType.POINTER, DatePointerType.ABOUT, "около")
        ...
        if ((tok is not None and (Utils.valToEnum(tok.termin.tag, DatePointerType)) == DatePointerType.SUMMER and t.morph.language.is_ru) and (isinstance(t, TextToken))): 
            ...
            if (str0_ != "ЛЕТОМ" and str0_ != "ЛЕТА" and str0_ != "ЛЕТО"): 
                tok = (None)
        if (tok is not None): 
            return DateItemToken._new838(t, tok.end_token, DateItemToken.DateItemType.POINTER, Utils.valToEnum(tok.termin.tag, DatePointerType))
        ...
        if (npt is not None): 
            ...
            if (npt.noun.is_value("КВАРТАЛ", None)): 
                ...
                return DateItemToken._new848(t, npt.end_token, DateItemToken.DateItemType.POINTER, DatePointerType.BEGIN, "начало")
            elif (npt.end_token.is_value("СЕРЕДИНА", None)): 
                return DateItemToken._new848(t, npt.end_token, DateItemToken.DateItemType.POINTER, DatePointerType.CENTER, "середина")
            elif (npt.end_token.is_value("КОНЕЦ", None) or npt.end_token.is_value("КІНЕЦЬ", None) or npt.end_token.is_value("НАПРИКІНЕЦЬ", None)): 
                return DateItemToken._new848(t, npt.end_token, DateItemToken.DateItemType.POINTER, DatePointerType.END, "конец")
            elif (npt.end_token.is_value("ВРЕМЯ", None) and len(npt.adjectives) > 0 and npt.end_token.previous.is_value("НАСТОЯЩЕЕ", None)): 
                return DateItemToken._new848(t, npt.end_token, DateItemToken.DateItemType.POINTER, DatePointerType.TODAY, "сегодня")
            ...
            if (typ_ != DateItemToken.DateItemType.NUMBER or detail_regime): 
                ...
                if (len(npt.adjectives) > 0): 
                    if (npt.adjectives[0].is_value("ПОСЛЕДНИЙ", "ОСТАННІЙ")): 
                        return DateItemToken._new860(t0, npt.end_token, typ_, (4 if typ_ == DateItemToken.DateItemType.QUARTAL else (9 if typ_ == DateItemToken.DateItemType.TENYEARS else 2)), DateItemToken.FirstLastTyp.LAST)
                ...
                while True:
                    ...
                    if (typ_ == DateItemToken.DateItemType.QUARTAL): 
                        ...
                        if ((ii < 1) or ii > 4): 
                            continue
                        return DateItemToken._new840(t0, npt.end_token, typ_, ii)
                ...
                if (ok and typ_ == DateItemToken.DateItemType.TENYEARS): 
                    return DateItemToken._new863(t0, npt.end_token, typ_, delta, True)
        term = t0.term
        if (not str.isalnum(term[0])): 
            if (t0.is_char_of(".\\/:") or t0.is_hiphen): 
                return DateItemToken._new864(t0, t0, DateItemToken.DateItemType.DELIM, term)
            elif (t0.is_char(',')): 
                return DateItemToken._new864(t0, t0, DateItemToken.DateItemType.DELIM, term)
            else: 
                return None
        if (term == "O" or term == "О"): 
            if ((isinstance(t.next0_, NumberToken)) and not t.is_whitespace_after and len(t.next0_.value) == 1): 
                return DateItemToken._new840(t, t.next0_, DateItemToken.DateItemType.NUMBER, t.next0_.int_value)
        if (str.isalpha(term[0])): 
            ...
            if (inf is not None and (isinstance(inf.termin.tag, int))): 
                return DateItemToken._new867(inf.begin_token, inf.end_token, DateItemToken.DateItemType.MONTH, inf.termin.tag, inf.termin.lang)
        return None
    ...
    def initialize() -> None:
        ...
        tt = Termin._new868("НОВАЯ ЭРА", MorphLang.RU, True, "НОВОЙ ЭРЫ")
        tt.add_variant("НАША ЭРА", True)
        tt.add_abridge("Н.Э.")
        DateItemToken.M_NEW_AGE.add(tt)
        tt = Termin._new868("НОВА ЕРА", MorphLang.UA, True, "НОВОЇ ЕРИ")
        ...
        DateItemToken.M_SEASONS.add(Termin._new721("ЗИМА", MorphLang.RU, True, DatePointerType.WINTER))
        DateItemToken.M_SEASONS.add(Termin._new721("WINTER", MorphLang.EN, True, DatePointerType.WINTER))
        t = Termin._new721("ВЕСНА", MorphLang.RU, True, DatePointerType.SPRING)
        t.add_variant("ПРОВЕСНА", True)
        DateItemToken.M_SEASONS.add(t)
        DateItemToken.M_SEASONS.add(Termin._new721("SPRING", MorphLang.EN, True, DatePointerType.SPRING))
        t = Termin._new721("ЛЕТО", MorphLang.RU, True, DatePointerType.SUMMER)
        DateItemToken.M_SEASONS.add(t)
        t = Termin._new721("ЛІТО", MorphLang.UA, True, DatePointerType.SUMMER)
        DateItemToken.M_SEASONS.add(t)
        t = Termin._new721("ОСЕНЬ", MorphLang.RU, True, DatePointerType.AUTUMN)
        DateItemToken.M_SEASONS.add(t)
        t = Termin._new721("AUTUMN", MorphLang.EN, True, DatePointerType.AUTUMN)
        DateItemToken.M_SEASONS.add(t)
        t = Termin._new721("ОСІНЬ", MorphLang.UA, True, DatePointerType.AUTUMN)
        DateItemToken.M_SEASONS.add(t)
        DateItemToken.M_MONTHES = TerminCollection()
        months = ["ЯНВАРЬ", "ФЕВРАЛЬ", "МАРТ", "АПРЕЛЬ", "МАЙ", "ИЮНЬ", "ИЮЛЬ", "АВГУСТ", "СЕНТЯБРЬ", "ОКТЯБРЬ", "НОЯБРЬ", "ДЕКАБРЬ"]
        i = 0
        while i < len(months): 
            t = Termin._new721(months[i], MorphLang.RU, True, i + 1)
            DateItemToken.M_MONTHES.add(t)
            i += 1
        months = ["СІЧЕНЬ", "ЛЮТИЙ", "БЕРЕЗЕНЬ", "КВІТЕНЬ", "ТРАВЕНЬ", "ЧЕРВЕНЬ", "ЛИПЕНЬ", "СЕРПЕНЬ", "ВЕРЕСЕНЬ", "ЖОВТЕНЬ", "ЛИСТОПАД", "ГРУДЕНЬ"]
        i = 0
        while i < len(months): 
            t = Termin._new721(months[i], MorphLang.UA, True, i + 1)
            DateItemToken.M_MONTHES.add(t)
            i += 1
        months = ["JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"]
        i = 0
        while i < len(months): 
            t = Termin._new721(months[i], MorphLang.EN, True, i + 1)
            DateItemToken.M_MONTHES.add(t)
            i += 1
        months = ["GENNAIO", "FEBBRAIO", "MARZO", "APRILE", "MAGGIO", "GUINGO", "LUGLIO", "AGOSTO", "SETTEMBRE", "OTTOBRE", "NOVEMBRE", "DICEMBRE"]
        i = 0
        while i < len(months): 
            t = Termin._new721(months[i], MorphLang.IT, True, i + 1)
            DateItemToken.M_MONTHES.add(t)
            i += 1
        ...
        te = Termin._new721("SUNDAY", MorphLang.EN, True, 7)
        te.add_abridge("SUN")
    ...
    def try_parse_list(t : 'Token', max_count : int=20) -> typing.List['DateItemToken']:
        ...
        while tt is not None:
            ...
            if (p0 is None): 
                ...
                if (((mc.is_adjective or mc.is_pronoun)) and not mc.is_verb): 
                    tt = tt.next0_
                    continue
    ...
    @staticmethod
    def _new837(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : 'FirstLastTyp', _arg5 : int) -> 'DateItemToken':
    ...
    @staticmethod
    def _new838(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : 'DatePointerType') -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.ptr = _arg4
        return res
    ...
    @staticmethod
    def _new839(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : 'MorphCollection') -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.morph = _arg5
    ...
    @staticmethod
    def _new840(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        return res
    ...
    @staticmethod
    def _new846(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : int) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.new_age = _arg5
    ...
    @staticmethod
    def _new848(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : 'DatePointerType', _arg5 : str) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.ptr = _arg4
    ...
    @staticmethod
    def _new860(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : object) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.tag = _arg5
    ...
    @staticmethod
    def _new863(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : bool) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.relate = _arg5
    ...
    @staticmethod
    def _new864(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : str) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
    ...
    @staticmethod
    def _new867(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : 'MorphLang') -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.lang = _arg5
