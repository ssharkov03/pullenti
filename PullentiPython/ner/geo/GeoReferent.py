...
class GeoReferent(Referent):
    ...
    def __is_city(v : str) -> bool:
        if ((((((((((((("поселок" in v or "селение" in v or "городок" in v) or "село" in v or "деревня" in v) or "станица" in v or "пункт" in v) or "станция" in v or "аул" in v) or "хутор" in v or "местечко" in v) or "урочище" in v or "усадьба" in v) or "починок" in v or "заимка" in v) or "аал" in v or "выселки" in v) or "арбан" in v or "місто" in v) or "селище" in v or "сіло" in v) or "станиця" in v or "станція" in v) or "city" in v or "municipality" in v) or "town" in v): 
            return True
        if ("порт" in v): 
            return True
    ...
    def _add_org_referent(self, org0_ : 'Referent') -> None:
        ...
        for s in org0_.slots: 
            if (s.type_name == "TYPE" and (isinstance(s.value, str))): 
                ...
                if (pref is None): 
                    pref = ty
                elif (str.isupper(ty[0])): 
                    if (str.islower(pref[0])): 
                        pref = ty
                    elif (len(pref) < len(ty)): 
                        pref = ty
                elif (str.islower(pref[0])): 
                    if (len(pref) > len(ty)): 
                        pref = ty
        for s in org0_.slots: 
            if (s.type_name == "NAME"): 
