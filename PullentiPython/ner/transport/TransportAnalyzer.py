...
from PullentiPython.ner.transport.internal.MetaTransport import MetaTransport
...
from PullentiPython.ner.transport.TransportReferent import TransportReferent
                        ...
                        if (s.type_name == TransportReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli2463 = RefOutArgWrapper(None)
                                    inoutres2464 = Utils.tryGetValue(objs_by_model, mod, wrapli2463)
                                    li = wrapli2463.value
                                    if (not inoutres2464): 
                                        li = list()
                            ...
                            obj_by_names.add(Termin._new264(str(s.value), rt.referent))
