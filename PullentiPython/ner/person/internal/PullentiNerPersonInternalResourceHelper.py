...
class PullentiNerPersonInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.ner.person.internal.properties', '.dat;.png;.txt')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.ner.person.internal.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.ner.person.internal.properties', n) as stream: 
