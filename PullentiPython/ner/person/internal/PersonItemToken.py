...
from PullentiPython.ner.person.internal.FioTemplateType import FioTemplateType
from PullentiPython.morph.MorphForm import MorphForm
...
from PullentiPython.ner.person.internal.ShortNameHelper import ShortNameHelper
...
class PersonItemToken(MetaToken):
    ...
    class MorphPersonItemVariant(MorphBaseInfo):
        ...
        @staticmethod
        def _new2349(_arg1 : str, _arg2 : 'MorphBaseInfo', _arg3 : bool, _arg4 : str) -> 'MorphPersonItemVariant':
    ...
    class MorphPersonItem:
        ...
        @staticmethod
        def _new2295(_arg1 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.is_has_std_postfix = _arg1
            return res
        
        @staticmethod
        def _new2303(_arg1 : str) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.term = _arg1
            return res
        
        @staticmethod
        def _new2306(_arg1 : str, _arg2 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
            res.term = _arg1
        ...
        @staticmethod
        def _new2338(_arg1 : bool) -> 'MorphPersonItem':
            res = PersonItemToken.MorphPersonItem()
    ...
    def __add_postfix_info(self, postfix : str, gen : 'MorphGender') -> None:
        ...
        if (self.firstname is not None): 
            ...
            self.firstname = PersonItemToken.MorphPersonItem._new2295(True)
            self.firstname.vars0_.append(PersonItemToken.MorphPersonItemVariant(self.value, MorphBaseInfo._new2296(gen), False))
    ...
    def try_attach_latin(t : 'Token') -> 'PersonItemToken':
        ...
        if (tt.term == "JR" or tt.term == "JNR" or tt.term == "JUNIOR"): 
            ...
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                t1 = tt.next0_
            return PersonItemToken._new2297(tt, t1, PersonItemToken.ItemType.SUFFIX, "JUNIOR")
        if ((tt.term == "SR" or tt.term == "SNR" or tt.term == "SENIOR") or tt.term == "FITZ" or tt.term == "FILS"): 
            t1 = tt
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                t1 = tt.next0_
            return PersonItemToken._new2297(tt, t1, PersonItemToken.ItemType.SUFFIX, "SENIOR")
        ...
        if (not initials and len(tt.term) == 2 and tt.chars.is_capital_upper): 
            if (not LanguageHelper.is_latin_vowel(tt.term[0]) and not LanguageHelper.is_latin_vowel(tt.term[1])): 
                initials = True
        if (initials): 
            rii = PersonItemToken._new2299(tt, tt, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
            if (tt.next0_ is not None and tt.next0_.is_char('.')): 
        ...
        if (tt.length_char == 1): 
            ...
            if (tt.next0_.is_char('.')): 
                return PersonItemToken._new2299(tt, tt.next0_, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
            ...
            if (not LanguageHelper.is_latin_vowel(tt.term[0]) or tt.whitespaces_after_count != 1): 
                ...
                if (nex is not None and nex.typ == PersonItemToken.ItemType.VALUE): 
                    return PersonItemToken._new2299(tt, tt, PersonItemToken.ItemType.INITIAL, tt.term, tt.chars)
                return None
            if (tt.term == "I"): 
                return None
            return PersonItemToken._new2299(tt, tt, PersonItemToken.ItemType.VALUE, tt.term, tt.chars)
        ...
        if (cla.is_proper_name or ((cla.is_proper and ((tt.morph.gender == MorphGender.MASCULINE or tt.morph.gender == MorphGender.FEMINIE))))): 
            res.firstname = PersonItemToken.MorphPersonItem._new2303(res.value)
        ...
        if (cla.is_proper_surname): 
            res.lastname = PersonItemToken.MorphPersonItem._new2303(res.value)
            for wf in tt.morph.items: 
                if (wf.is_in_dictionary): 
        ...
        if (ots is not None): 
            if (ots[0].termin.ignore_terms_order): 
                return PersonItemToken._new2305(ots[0].begin_token, ots[0].end_token, PersonItemToken.ItemType.REFERENT, Utils.asObjectOrNull(ots[0].item.tag, PersonReferent), ots[0].morph)
            res.lastname = PersonItemToken.MorphPersonItem._new2306(ots[0].termin.canonic_text, True)
            for ot in ots: 
                if (ot.termin is not None): 
                    mi = ot.morph
                    if (ot.termin.gender == MorphGender.MASCULINE or ot.termin.gender == MorphGender.FEMINIE): 
                        mi = MorphBaseInfo._new2296(ot.termin.gender)
        ...
        if (res.value.startswith("MAC")): 
            ...
            res.lastname = PersonItemToken.MorphPersonItem._new2295(True)
            res.lastname.vars0_.append(PersonItemToken.MorphPersonItemVariant(res.value, MorphBaseInfo(), True))
        return res
    ...
    def try_attach(t : 'Token', attrs : 'ParseAttr'=ParseAttr.NO, prev_list : typing.List['PersonItemToken']=None) -> 'PersonItemToken':
        ...
        if ((((isinstance(t, TextToken)) and t.length_char == 2 and t.term == "JI") and t.chars.is_all_upper and not t.is_whitespace_after) and t.next0_ is not None and t.next0_.is_char('.')): 
            re1 = PersonItemToken._new2297(t, t.next0_, PersonItemToken.ItemType.INITIAL, "Л")
        ...
        if ((((((isinstance(t, TextToken)) and t.length_char == 1 and t.term == "J") and t.chars.is_all_upper and not t.is_whitespace_after) and (isinstance(t.next0_, NumberToken)) and t.next0_.value == "1") and t.next0_.typ == NumberSpellingType.DIGIT and t.next0_.next0_ is not None) and t.next0_.next0_.is_char('.')): 
            re1 = PersonItemToken._new2297(t, t.next0_.next0_, PersonItemToken.ItemType.INITIAL, "Л")
            re1.chars.is_cyrillic_letter = True
            re1.chars.is_all_upper = True
            return re1
        if ((((((isinstance(t, TextToken)) and t.length_char == 1 and t.term == "I") and t.chars.is_all_upper and not t.is_whitespace_after) and (isinstance(t.next0_, NumberToken)) and t.next0_.value == "1") and t.next0_.typ == NumberSpellingType.DIGIT and t.next0_.next0_ is not None) and t.next0_.next0_.is_char('.')): 
            if (prev_list is not None and prev_list[0].chars.is_cyrillic_letter): 
                re1 = PersonItemToken._new2297(t, t.next0_.next0_, PersonItemToken.ItemType.INITIAL, "П")
                re1.chars.is_cyrillic_letter = True
                re1.chars.is_all_upper = True
                return re1
        ...
        if (t.chars.is_latin_letter and (((attrs) & (PersonItemToken.ParseAttr.CANBELATIN))) != (PersonItemToken.ParseAttr.NO)): 
            ...
            if (ots is not None and (isinstance(t, TextToken))): 
                if (ots[0].termin.ignore_terms_order): 
                    return PersonItemToken._new2305(ots[0].begin_token, ots[0].end_token, PersonItemToken.ItemType.REFERENT, Utils.asObjectOrNull(ots[0].item.tag, PersonReferent), ots[0].morph)
                res = PersonItemToken._new2313(ots[0].begin_token, ots[0].end_token, t.term, ots[0].chars)
                res.lastname = PersonItemToken.MorphPersonItem._new2306(ots[0].termin.canonic_text, True)
                for ot in ots: 
                    if (ot.termin is not None): 
                        mi = ot.morph
        ...
        if (((isinstance(t, NumberToken)) and t.length_char == 1 and (((attrs) & (PersonItemToken.ParseAttr.CANINITIALBEDIGIT))) != (PersonItemToken.ParseAttr.NO)) and t.next0_ is not None and t.next0_.is_char_of(".„")): 
            if (t.value == "1"): 
                return PersonItemToken._new2299(t, t.next0_, PersonItemToken.ItemType.INITIAL, "І", CharsInfo._new2316(True))
            if (t.value == "0"): 
                return PersonItemToken._new2299(t, t.next0_, PersonItemToken.ItemType.INITIAL, "О", CharsInfo._new2316(True))
            if (t.value == "3"): 
                return PersonItemToken._new2299(t, t.next0_, PersonItemToken.ItemType.INITIAL, "З", CharsInfo._new2316(True))
        if ((((isinstance(t, NumberToken)) and t.length_char == 1 and (((attrs) & (PersonItemToken.ParseAttr.CANINITIALBEDIGIT))) != (PersonItemToken.ParseAttr.NO)) and t.next0_ is not None and t.next0_.chars.is_all_lower) and not t.is_whitespace_after and t.next0_.length_char > 2): 
            ...
            if (num == "3" and t.next0_.chars.is_cyrillic_letter): 
                return PersonItemToken._new2299(t, t.next0_, PersonItemToken.ItemType.VALUE, "З" + t.next0_.term, CharsInfo._new2322(True, True))
            if (num == "0" and t.next0_.chars.is_cyrillic_letter): 
                return PersonItemToken._new2299(t, t.next0_, PersonItemToken.ItemType.VALUE, "О" + t.next0_.term, CharsInfo._new2322(True, True))
        if (((((isinstance(t, TextToken)) and t.length_char == 1 and t.chars.is_letter) and t.chars.is_all_upper and (t.whitespaces_after_count < 2)) and (isinstance(t.next0_, TextToken)) and t.next0_.length_char == 1) and t.next0_.chars.is_all_lower): 
            ...
            while tt is not None and tt.end_char <= t1.end_char: 
                ...
                tt = tt.next0_
            res = PersonItemToken._new2297(t, t1, PersonItemToken.ItemType.VALUE, Utils.toStringStringIO(val))
            res.chars = CharsInfo._new2327(True, is_cyr, not is_cyr, True)
            return res
        if ((((attrs) & (PersonItemToken.ParseAttr.MUSTBEITEMALWAYS))) != (PersonItemToken.ParseAttr.NO) and (isinstance(t, TextToken)) and not t.chars.is_all_lower): 
            res = PersonItemToken._new2328(t, t, t.term)
            return res
        if (((t.chars.is_all_upper and (isinstance(t, TextToken)) and t.length_char == 1) and prev_list is not None and len(prev_list) > 0) and (t.whitespaces_before_count < 2) and prev_list[0].chars.is_capital_upper): 
            ...
            if ((last.typ == PersonItemToken.ItemType.VALUE and last.lastname is not None and last.lastname.is_in_dictionary) and len(prev_list) == 1): 
                ...
                ok = True
            if (ok): 
                return PersonItemToken._new2329(t, t, t.term, PersonItemToken.ItemType.INITIAL)
        return None
    ...
    def __try_attach(t : 'Token', attrs : 'ParseAttr', prev_list : typing.List['PersonItemToken']=None) -> 'PersonItemToken':
        ...
        if (tt is None): 
            if (t.chars.is_letter and t.chars.is_capital_upper and (isinstance(t, ReferentToken))): 
                ...
                if (rt.begin_token == rt.end_token and not (isinstance(rt.referent, PersonReferent))): 
                    ...
                    if (res0 is None): 
                        res0 = PersonItemToken._new2330(rt, rt, rt.referent.to_string_ex(True, t.kit.base_language, 0).upper(), rt.chars, rt.morph)
                        res0.lastname = PersonItemToken.MorphPersonItem._new2303(res0.value)
                    else: 
        ...
        if (tt.length_char == 1 or tt.term == "ДЖ"): 
            if (tt.next0_ is None): 
                return None
            ini = tt.term
            ci = CharsInfo._new2332(tt.chars.value)
            ...
            if (tt.next0_.is_char('.')): 
                return PersonItemToken._new2299(tt, tt.next0_, PersonItemToken.ItemType.INITIAL, ini, ci)
            if ((tt.next0_.is_char_of(",;„") and prev_list is not None and len(prev_list) > 0) and prev_list[len(prev_list) - 1].typ == PersonItemToken.ItemType.INITIAL): 
                return PersonItemToken._new2299(tt, tt, PersonItemToken.ItemType.INITIAL, ini, ci)
        ...
        if (ots is not None): 
            ...
            if (ots[0].begin_token == ots[0].end_token and mc.is_proper_name and not mc.is_proper_surname): 
                ots = (None)
        if (ots is not None): 
            res = PersonItemToken._new2313(ots[0].begin_token, ots[0].end_token, tt.term, ots[0].chars)
            res.lastname = PersonItemToken.MorphPersonItem._new2338(True)
            ...
            res = PersonItemToken._new2330(t, t, tt.term, tt.chars, tt.morph)
        ...
        for wff in tt.morph.items: 
            ...
            if (wf.class0_.is_proper_surname or sur_prefix is not None): 
                if (res.lastname is None): 
                    res.lastname = PersonItemToken.MorphPersonItem._new2303(tt.term)
                if (adj is not None): 
                ...
                if (wf.is_in_dictionary and v.gender == MorphGender.UNDEFINED and wf.gender == MorphGender.UNDEFINED): 
                    ...
                    vv = PersonItemToken.MorphPersonItemVariant(t.term, MorphBaseInfo._new2250(MorphCase.NOMINATIVE, MorphGender.MASCULINE), True)
                    vv.value = t.term
                ...
                if (tt.term.endswith("ИХ") or tt.term.endswith("ЫХ")): 
                    if (res.lastname.vars0_[0].value != tt.term): 
                        res.lastname.vars0_.insert(0, PersonItemToken.MorphPersonItemVariant(tt.term, MorphBaseInfo._new2345(MorphCase.ALL_CASES, Utils.valToEnum((MorphGender.MASCULINE) | (MorphGender.FEMINIE), MorphGender), MorphClass._new2344(True)), True))
            ...
            if (wf.class0_.is_proper_name and wf.number != MorphNumber.PLURAL): 
                ...
                if (ok): 
                    if (res.firstname is None): 
                        res.firstname = PersonItemToken.MorphPersonItem._new2303(tt.term)
            ...
            if (not PersonItemToken.MorphPersonItem.ends_with_std_surname(tt.term)): 
                if (wf.class0_.is_proper_secname): 
                    if (res.middlename is None): 
                        res.middlename = PersonItemToken.MorphPersonItem._new2303(tt.term)
                ...
                if (g != MorphGender.MASCULINE and g != MorphGender.FEMINIE): 
                    ...
                    for kp in di: 
                        if (fi): 
                            ...
                            mi = MorphBaseInfo._new2296(kp.gender)
                            res.firstname.vars0_.append(PersonItemToken.MorphPersonItemVariant._new2349(kp.name, mi, False, val))
                else: 
                    cou = 0
                    for kp in di: 
                        if (kp.gender == g): 
                            ...
                            if (cou < 2): 
                                ...
                                res.firstname.vars0_.insert(i + 1, PersonItemToken.MorphPersonItemVariant._new2349(kp.name, res.firstname.vars0_[i], False, val))
        ...
        if ((t is not None and t.is_char('.') and pit.typ == PersonItemToken.ItemType.VALUE) and pit.length_char > 3): 
            ...
            if (str.isupper(str0_[0]) and str.isupper(str0_[len(str0_) - 1])): 
                ...
                if (ok): 
                    ...
                    pit2 = PersonItemToken._new2297(t, t, PersonItemToken.ItemType.INITIAL, str0_[len(str0_) - 1:])
                    res.append(pit2)
                    t = t.next0_
        zap = False
        while t is not None: 
                ...
                if (pit1.typ == PersonItemToken.ItemType.INITIAL): 
                    if (pit1.chars.is_cyrillic_letter): 
                        ...
                        if (v != (chr(0))): 
                            pit1.value = "{0}".format(v)
                            ok = True
                            pit1.chars = CharsInfo._new2316(True)
                        elif (pit.typ == PersonItemToken.ItemType.INITIAL): 
                            v = LanguageHelper.get_lat_for_cyr(pit.value[0])
                            if (v != (chr(0))): 
                                pit.value = "{0}".format(v)
                                ok = True
                                pit.chars = CharsInfo._new2352(True)
                                pit = pit1
                elif (pit.typ == PersonItemToken.ItemType.INITIAL): 
        ...
        while True:
            ...
            if (res[i].firstname is not None and res[i].begin_token.is_value("СВЕТА", None)): 
                ...
                if (res[i].lastname is None): 
                    res[i].lastname = PersonItemToken.MorphPersonItem._new2295(True)
    ...
    def try_parse_person(t : 'Token', prev_pers_template : 'FioTemplateType'=FioTemplateType.UNDEFINED) -> 'ReferentToken':
                ...
                if (tt1 is not None and (tt1.whitespaces_before_count < 2)): 
                    ...
                    if (pits0 is not None and pits0[0].typ == PersonItemToken.ItemType.INITIAL): 
                        ...
                        if (str0_ is not None and str0_.startswith(pits0[0].value)): 
                            res = ReferentToken._new2357(rt.referent, t, pits0[0].end_token, FioTemplateType.SURNAMEI)
                    ...
                    if (((((isinstance(tt1, TextToken)) and tt1.length_char == 1 and tt1.chars.is_all_upper) and tt1.chars.is_cyrillic_letter and (isinstance(tt1.next0_, TextToken))) and (tt1.whitespaces_after_count < 2) and tt1.next0_.length_char == 1) and tt1.next0_.chars.is_all_upper and tt1.next0_.chars.is_cyrillic_letter): 
                        ...
                        if (str0_ is not None and str0_.startswith(tt1.term)): 
                            ...
                            if (str2 is None or str2.startswith(tt1.next0_.term)): 
                                res = ReferentToken._new2357(rt.referent, t, tt1.next0_, FioTemplateType.NAMEISURNAME)
                                if (str2 is None): 
        ...
        if (pits is not None and prev_pers_template != FioTemplateType.UNDEFINED and pits[0].typ == PersonItemToken.ItemType.VALUE): 
            ...
            if (((isinstance(tt1, TextToken)) and tt1.chars.is_letter and tt1.chars.is_all_upper) and tt1.length_char == 1 and (tt1.whitespaces_before_count < 2)): 
                ii = PersonItemToken._new2299(tt1, tt1, PersonItemToken.ItemType.INITIAL, tt1.term, tt1.chars)
                pits.append(ii)
        ...
        if (pits is not None and len(pits) == 1 and pits[0].typ == PersonItemToken.ItemType.VALUE): 
            ...
            if (((isinstance(tt, TextToken)) and tt.length_char == 2 and tt.chars.is_all_upper) and tt.chars.is_cyrillic_letter): 
                ...
                res = ReferentToken._new2357(pers, t, tt, FioTemplateType.SURNAMEII)
                if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                    tt = tt.next0_
                    res.end_token = tt
                res.data = t.kit.get_analyzer_data_by_analyzer_name(PersonAnalyzer.ANALYZER_NAME)
            ...
            if ((((((isinstance(tt, TextToken)) and (tt.whitespaces_before_count < 2) and tt.length_char == 1) and tt.chars.is_all_upper and tt.chars.is_cyrillic_letter) and (isinstance(tt.next0_, TextToken)) and (tt.whitespaces_after_count < 2)) and tt.next0_.length_char == 1 and tt.next0_.chars.is_all_upper) and tt.next0_.chars.is_cyrillic_letter): 
                ...
                res = ReferentToken._new2357(pers, t, tt.next0_, FioTemplateType.SURNAMEII)
            ...
            if (comma and tt is not None and (tt.whitespaces_before_count < 2)): 
                ...
                if (pits1 is not None and len(pits1) > 0 and pits1[0].typ == PersonItemToken.ItemType.INITIAL): 
                    ...
                    res = ReferentToken._new2357(pers, t, pits1[0].end_token, FioTemplateType.SURNAMEI)
    ...
    @staticmethod
    def _new2297(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : str) -> 'PersonItemToken':
    ...
    @staticmethod
    def _new2299(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : str, _arg5 : 'CharsInfo') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        res.chars = _arg5
    ...
    @staticmethod
    def _new2305(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ItemType', _arg4 : 'PersonReferent', _arg5 : 'MorphCollection') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.referent = _arg4
        res.morph = _arg5
    ...
    @staticmethod
    def _new2313(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        return res
    ...
    @staticmethod
    def _new2328(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        return res
    
    @staticmethod
    def _new2329(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'ItemType') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.typ = _arg4
        return res
    ...
    @staticmethod
    def _new2330(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'CharsInfo', _arg5 : 'MorphCollection') -> 'PersonItemToken':
        res = PersonItemToken(_arg1, _arg2)
        res.value = _arg3
        res.chars = _arg4
        res.morph = _arg5
