...
class AddressReferent(Referent):
    ...
    def flat(self) -> str:
        """ Квартира """
        res = self.get_string_value(AddressReferent.ATTR_FLAT)
        if (res == "0"): 
            return None
        return res
    @flat.setter
    ...
    def __out_house(self, res : io.StringIO) -> None:
        ...
        if (self.house is not None): 
            ...
            print(" {0}".format(self.house_or_plot), end="", file=res, flush=True)
        ...
        if (self.room is not None): 
            print(" ком.{0}".format(self.room), end="", file=res, flush=True)
