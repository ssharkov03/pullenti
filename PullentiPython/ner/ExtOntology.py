...
from PullentiPython.ner.ExtOntologyItem import ExtOntologyItem
...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new2583(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new2583(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer2585 = RefOutArgWrapper(None)
        inoutres2586 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer2585)
        analyzer = wrapanalyzer2585.value
        if (not inoutres2586): 
            return None
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer2587 = RefOutArgWrapper(None)
        inoutres2588 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer2587)
        analyzer = wrapanalyzer2587.value
        if (not inoutres2588): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa2589 = RefOutArgWrapper(None)
        inoutres2590 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa2589)
        a = wrapa2589.value
        if (not inoutres2590): 
            return None
    ...
    def __init_hash(self) -> None:
        ...
        for it in self.items: 
            if (it.referent is not None): 
                ont = None
                wrapont2592 = RefOutArgWrapper(None)
                inoutres2593 = Utils.tryGetValue(self.__m_hash, it.referent.type_name, wrapont2592)
                ont = wrapont2592.value
                if (not inoutres2593): 
                    ont = IntOntologyCollection._new2591(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto2594 = RefOutArgWrapper(None)
        inoutres2595 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto2594)
        onto = wraponto2594.value
        if (not inoutres2595): 
            return None
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto2596 = RefOutArgWrapper(None)
        inoutres2597 = Utils.tryGetValue(self.__m_hash, type_name, wraponto2596)
        onto = wraponto2596.value
        if (not inoutres2597): 
            return None
