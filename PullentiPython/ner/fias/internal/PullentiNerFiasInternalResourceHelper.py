...
class PullentiNerFiasInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.address.internal.gar.properties', '.png')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.address.internal.gar.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.address.internal.gar.properties', n) as stream: 
    ...
    def get_string(name : str) -> str:
        arr = PullentiNerFiasInternalResourceHelper.get_bytes(name)
        if (arr is None): 
            return None
