...
class NumberHelper:
    ...
    def __try_parse(token : 'Token', prev_val : 'NumberToken'=None) -> 'NumberToken':
        ...
        if (val is not None): 
                ...
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii686 = RefOutArgWrapper(0)
                            inoutres687 = Utils.tryParseInt(ss, wrapii686)
                            ii = wrapii686.value
                            if (not inoutres687): 
                                break
                            val2 += ss
            ...
            for k in range(4):
                if ((isinstance(et.next0_, TextToken)) and et.next0_.chars.is_letter): 
                    ...
                    break
            res = NumberToken._new688(token, et, val, typ, mc)
        ...
        while t is not None: 
            ...
            if (num is None): 
                break
            wrapj689 = RefOutArgWrapper(0)
            inoutres690 = Utils.tryParseInt(str(num.termin.tag), wrapj689)
            j = wrapj689.value
            if (not inoutres690): 
                break
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
            ...
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    ...
                    if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ"): 
                        return NumberToken._new688(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new688(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ...
            for i in range(len(ss) - 2, 2, -1):
                ...
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new688(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new696(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new696(tt, tt, mc)
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new698(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new696(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            return MetaToken._new696(tt, tt, MorphCollection._new700(MorphClass.ADJECTIVE))
        return None
    
    @staticmethod
    def __try_parse_float(t : 'NumberToken', d : float, attrs : 'NumberParseAttr') -> 'Token':
        from PullentiPython.ner.core.internal.NumberExHelper import NumberExHelper
        d.value = (0)
        ...
        while True:
            ...
            if (t1.next0_ is None): 
                break
            if (((isinstance(t1.next0_, NumberToken)) and (t1.whitespaces_after_count < 6) and t1.next0_.typ == NumberSpellingType.DIGIT) and t1.next0_.length_char == 3): 
                if (ns is None): 
                    ns = list()
                    ns.append(t)
                    sps = list()
    ...
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            ...
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd704 = RefOutArgWrapper(0)
                    inoutres705 = Utils.tryParseFloat(str0_, wrapdd704)
                    dd = wrapdd704.value
                    if (inoutres705): 
                        pass
                    else: 
                        wrapdd702 = RefOutArgWrapper(0)
                        inoutres703 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd702)
                        dd = wrapdd702.value
                        if (inoutres703): 
                            pass
                        else: 
                ...
                return res0
        d = 0
        wrapd707 = RefOutArgWrapper(0)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd707, attrs)
        d = wrapd707.value
        if (tt is None): 
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new706(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d)
    ...
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        n = 0
        wrapn709 = RefOutArgWrapper(0)
        Utils.tryParseInt(num_val, wrapn709)
        n = wrapn709.value
        res = MorphBaseInfo._new698(cla)
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres714 = RefOutArgWrapper(0)
        inoutres715 = Utils.tryParseFloat(str0_, wrapres714)
        res = wrapres714.value
        if (inoutres715): 
            return res
        wrapres712 = RefOutArgWrapper(0)
        inoutres713 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres712)
        res = wrapres712.value
        if (inoutres713): 
            return res
        wrapres710 = RefOutArgWrapper(0)
        inoutres711 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres710)
