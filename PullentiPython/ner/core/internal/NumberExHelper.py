...
from PullentiPython.ner.core.NumberParseAttr import NumberParseAttr
...
from PullentiPython.ner.core.NumberExToken import NumberExToken
...
class NumberExHelper:
    ...
    def try_parse_number_with_postfix(t : 'Token') -> 'NumberExToken':
        ...
        if (nt is None): 
            if ((not (isinstance(t.previous, NumberToken)) and t.is_char('(') and (isinstance(t.next0_, NumberToken))) and t.next0_.next0_ is not None and t.next0_.next0_.is_char(')')): 
                ...
                if (toks1 is not None and (Utils.valToEnum(toks1.termin.tag, NumberExType)) == NumberExType.MONEY): 
                    ...
                    res = NumberExToken._new589(t, toks1.end_token, nt0.value, nt0.typ, NumberExType.MONEY, nt0.real_value, toks1.begin_token.morph)
            ...
            while True:
                ...
                if (lii is not None and len(lii) > 0): 
                    re = NumberExToken._new590(t, t, str(li[0].tag), NumberSpellingType.WORDS, Utils.valToEnum(lii[0].tag, NumberExType), t.morph)
                    ...
                    if (toks1 is not None): 
                        if ((Utils.valToEnum(toks1.termin.tag, NumberExType)) == NumberExType.MONEY): 
                            if (toks1.end_token.next0_ is not None and toks1.end_token.next0_.is_char(')')): 
                                res = NumberExToken._new591(t, toks1.end_token.next0_, nt.value, nt.typ, NumberExType.MONEY, f, altf, toks1.begin_token.morph)
                                return NumberExHelper.__correct_money(res, toks1.begin_token)
        ...
        if (is_dollar is not None): 
            ...
            if (not te.is_whitespace_after and (isinstance(te.next0_, TextToken))): 
                if (te.next0_.is_value("M", None)): 
                    ...
                    altf *= (1000000000)
                    te = te.next0_
            return NumberExToken._new592(t0, te, "", nt.typ, NumberExType.MONEY, f, altf, is_dollar)
        ...
        if (toks is None and t1.is_char('р')): 
            ...
            while True:
                ...
                if (ttt.is_value("СУММА", None) or ttt.is_value("НАЛИЧНЫЙ", None) or ttt.is_value("БАЛАНС", None)): 
                    ...
                    continue
                toks = TerminToken._new593(t1, t1, NumberExHelper._m_postfixes.find_termins_by_canonic_text("RUB")[0])
                if (t1.next0_ is not None and t1.next0_.is_char('.')): 
                    toks.end_token = t1.next0_
                ty = Utils.valToEnum(toks.termin.tag, NumberExType)
                return NumberExToken._new594(t, toks.end_token, nt.value, nt.typ, ty, f, altf, toks.begin_token.morph, "RUB")
        if (toks is not None): 
            t1 = toks.end_token
            ...
            if (toks.begin_token == t1): 
                if (t1.morph.class0_.is_preposition or t1.morph.class0_.is_conjunction): 
                    if (t1.is_whitespace_before and t1.is_whitespace_after): 
                        return None
            ty = Utils.valToEnum(toks.termin.tag, NumberExType)
            res = NumberExToken._new591(t, t1, nt.value, nt.typ, ty, f, altf, toks.begin_token.morph)
        ...
        if (t1.next0_ is not None and ((t1.morph.class0_.is_preposition or t1.morph.class0_.is_conjunction))): 
            if (t1.is_value("НА", None)): 
                ...
                if (nn is not None): 
                    return NumberExToken._new596(t, t, nt.value, nt.typ, nn.ex_typ, f, altf, nn.ex_typ2, nn.ex_typ_param)
        if (not t1.is_whitespace_after and (isinstance(t1.next0_, NumberToken)) and (isinstance(t1, TextToken))): 
            ...
            if (ty != NumberExType.UNDEFINED): 
                return NumberExToken._new597(t, t1, nt.value, nt.typ, ty, f, altf, True)
        return None
    ...
    def try_attach_postfix_only(t : 'Token') -> 'NumberExToken':
        ...
        res = None
        if (tok is not None): 
            res = NumberExToken._new598(t, tok.end_token, "", NumberSpellingType.DIGIT, Utils.valToEnum(tok.termin.tag, NumberExType), tok.termin)
        else: 
    ...
    def __attach_spec_postfix(t : 'Token') -> 'NumberExToken':
        ...
        if (money is not None): 
            return NumberExToken._new599(t, t, "", NumberSpellingType.DIGIT, NumberExType.MONEY, money)
        return None
    ...
    def __correct_ext_types(ex : 'NumberExToken') -> None:
        ...
        if (t is None): 
            return
        ty = ex.ex_typ
        wrapty601 = RefOutArgWrapper(ty)
        tt = NumberExHelper.__corr_ex_typ2(t, wrapty601)
        ty = wrapty601.value
        if (tt is not None): 
            ex.ex_typ = ty
            ex.end_token = tt
        ...
        if (tok is not None and (((Utils.valToEnum(tok.termin.tag, NumberExType)) != NumberExType.MONEY))): 
            ...
            ty = ex.ex_typ2
            wrapty600 = RefOutArgWrapper(ty)
            tt = NumberExHelper.__corr_ex_typ2(ex.end_token.next0_, wrapty600)
            ty = wrapty600.value
            if (tt is not None): 
                ex.ex_typ2 = ty
                ex.end_token = tt
    ...
    def _initialize() -> None:
        ...
        t = Termin._new602("КВАДРАТНЫЙ МЕТР", MorphLang.RU, True, "кв.м.", NumberExType.METER2)
        t.add_abridge("КВ.МЕТР")
        t.add_abridge("КВ.МЕТРА")
        t.add_abridge("КВ.М.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КВАДРАТНИЙ МЕТР", MorphLang.UA, True, "КВ.М.", NumberExType.METER2)
        t.add_abridge("КВ.МЕТР")
        t.add_abridge("КВ.МЕТРА")
        t.add_abridge("КВ.М.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КВАДРАТНЫЙ КИЛОМЕТР", MorphLang.RU, True, "кв.км.", NumberExType.KILOMETER2)
        t.add_variant("КВАДРАТНИЙ КІЛОМЕТР", True)
        t.add_abridge("КВ.КМ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ГЕКТАР", MorphLang.RU, True, "га", NumberExType.GEKTAR)
        t.add_abridge("ГА")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("АР", MorphLang.RU, True, "ар", NumberExType.AR)
        t.add_variant("СОТКА", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КУБИЧЕСКИЙ МЕТР", MorphLang.RU, True, "куб.м.", NumberExType.METER3)
        t.add_variant("КУБІЧНИЙ МЕТР", True)
        t.add_abridge("КУБ.МЕТР")
        t.add_abridge("КУБ.М.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МЕТР", MorphLang.RU, True, "м.", NumberExType.METER)
        t.add_abridge("М.")
        t.add_abridge("M.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МЕТРОВЫЙ", MorphLang.RU, True, "м.", NumberExType.METER)
        t.add_variant("МЕТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИМЕТР", MorphLang.RU, True, "мм.", NumberExType.MILLIMETER)
        t.add_abridge("ММ")
        t.add_abridge("MM")
        t.add_variant("МІЛІМЕТР", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИМЕТРОВЫЙ", MorphLang.RU, True, "мм.", NumberExType.MILLIMETER)
        t.add_variant("МІЛІМЕТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("САНТИМЕТР", MorphLang.RU, True, "см.", NumberExType.SANTIMETER)
        t.add_abridge("СМ")
        t.add_abridge("CM")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("САНТИМЕТРОВЫЙ", MorphLang.RU, True, "см.", NumberExType.SANTIMETER)
        t.add_variant("САНТИМЕТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КВАДРАТНЫЙ САНТИМЕТР", MorphLang.RU, True, "кв.см.", NumberExType.SANTIMETER2)
        t.add_variant("КВАДРАТНИЙ САНТИМЕТР", True)
        t.add_abridge("КВ.СМ.")
        t.add_abridge("СМ.КВ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КУБИЧЕСКИЙ САНТИМЕТР", MorphLang.RU, True, "куб.см.", NumberExType.SANTIMETER3)
        t.add_variant("КУБІЧНИЙ САНТИМЕТР", True)
        t.add_abridge("КУБ.САНТИМЕТР")
        t.add_abridge("КУБ.СМ.")
        t.add_abridge("СМ.КУБ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КИЛОМЕТР", MorphLang.RU, True, "км.", NumberExType.KILOMETER)
        t.add_abridge("КМ")
        t.add_abridge("KM")
        t.add_variant("КІЛОМЕТР", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КИЛОМЕТРОВЫЙ", MorphLang.RU, True, "км.", NumberExType.KILOMETER)
        t.add_variant("КІЛОМЕТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЯ", MorphLang.RU, True, "миль", NumberExType.KILOMETER)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ГРАММ", MorphLang.RU, True, "гр.", NumberExType.GRAMM)
        t.add_abridge("ГР")
        t.add_abridge("Г")
        t.add_variant("ГРАМ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ГРАММОВЫЙ", MorphLang.RU, True, "гр.", NumberExType.GRAMM)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КИЛОГРАММ", MorphLang.RU, True, "кг.", NumberExType.KILOGRAM)
        t.add_abridge("КГ")
        t.add_variant("КІЛОГРАМ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КИЛОГРАММОВЫЙ", MorphLang.RU, True, "кг.", NumberExType.KILOGRAM)
        t.add_variant("КІЛОГРАМОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИГРАММ", MorphLang.RU, True, "мг.", NumberExType.MILLIGRAM)
        t.add_abridge("МГ")
        t.add_variant("МІЛІГРАМ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИГРАММОВЫЙ", MorphLang.RU, True, "мг.", NumberExType.MILLIGRAM)
        t.add_variant("МИЛЛИГРАМОВЫЙ", True)
        t.add_variant("МІЛІГРАМОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ТОННА", MorphLang.RU, True, "т.", NumberExType.TONNA)
        t.add_abridge("Т")
        t.add_abridge("T")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ТОННЫЙ", MorphLang.RU, True, "т.", NumberExType.TONNA)
        t.add_variant("ТОННИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ЛИТР", MorphLang.RU, True, "л.", NumberExType.LITR)
        t.add_abridge("Л")
        t.add_variant("ЛІТР", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ЛИТРОВЫЙ", MorphLang.RU, True, "л.", NumberExType.LITR)
        t.add_variant("ЛІТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИЛИТР", MorphLang.RU, True, "мл.", NumberExType.MILLILITR)
        t.add_abridge("МЛ")
        t.add_variant("МІЛІЛІТР", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИЛЛИЛИТРОВЫЙ", MorphLang.RU, True, "мл.", NumberExType.MILLILITR)
        t.add_variant("МІЛІЛІТРОВИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ЧАС", MorphLang.RU, True, "ч.", NumberExType.HOUR)
        t.add_abridge("Ч.")
        t.add_variant("ГОДИНА", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МИНУТА", MorphLang.RU, True, "мин.", NumberExType.MINUTE)
        t.add_abridge("МИН.")
        t.add_variant("ХВИЛИНА", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("СЕКУНДА", MorphLang.RU, True, "сек.", NumberExType.SECOND)
        t.add_abridge("СЕК.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ГОД", MorphLang.RU, True, "г.", NumberExType.YEAR)
        t.add_abridge("Г.")
        t.add_abridge("ЛЕТ")
        t.add_variant("ЛЕТНИЙ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("МЕСЯЦ", MorphLang.RU, True, "мес.", NumberExType.MONTH)
        t.add_abridge("МЕС.")
        t.add_variant("МЕСЯЧНЫЙ", True)
        t.add_variant("КАЛЕНДАРНЫЙ МЕСЯЦ", True)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ДЕНЬ", MorphLang.RU, True, "дн.", NumberExType.DAY)
        t.add_abridge("ДН.")
        ...
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("НЕДЕЛЯ", MorphLang.RU, True, "нед.", NumberExType.WEEK)
        t.add_variant("НЕДЕЛЬНЫЙ", True)
        t.add_variant("КАЛЕНДАРНАЯ НЕДЕЛЯ", False)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ПРОЦЕНТ", MorphLang.RU, True, "%", NumberExType.PERCENT)
        t.add_variant("%", False)
        t.add_variant("ПРОЦ", True)
        t.add_abridge("ПРОЦ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ШТУКА", MorphLang.RU, True, "шт.", NumberExType.SHUK)
        t.add_variant("ШТ", False)
        t.add_abridge("ШТ.")
        t.add_abridge("ШТ-К")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("УПАКОВКА", MorphLang.RU, True, "уп.", NumberExType.UPAK)
        ...
        t.add_abridge("УП-КА")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("РУЛОН", MorphLang.RU, True, "рулон", NumberExType.RULON)
        t.add_variant("РУЛ", True)
        t.add_abridge("РУЛ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("НАБОР", MorphLang.RU, True, "набор", NumberExType.NABOR)
        t.add_variant("НАБ", True)
        t.add_abridge("НАБ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("КОМПЛЕКТ", MorphLang.RU, True, "компл.", NumberExType.KOMPLEKT)
        t.add_variant("КОМПЛ", True)
        t.add_abridge("КОМПЛ.")
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ПАРА", MorphLang.RU, True, "пар", NumberExType.PARA)
        NumberExHelper._m_postfixes.add(t)
        t = Termin._new602("ФЛАКОН", MorphLang.RU, True, "флак.", NumberExType.FLAKON)
        ...
        t = Termin._new290("УСЛОВНАЯ ЕДИНИЦА", "УЕ", NumberExType.MONEY)
        t.add_abridge("У.Е.")
        t.add_abridge("У.E.")
        t.add_abridge("Y.Е.")
        t.add_abridge("Y.E.")
        ...
        for k in range(3):
            ...
            for line0 in Utils.splitString(str0_, '\n', False): 
                ...
                if (i < 2): 
                    continue
                wrapnum647 = RefOutArgWrapper(0)
                inoutres648 = Utils.tryParseInt(parts[3][0:0+i], wrapnum647)
                num = wrapnum647.value
                if (not inoutres648): 
                    continue
                vv = parts[3][i:].strip()
                t = Termin()
