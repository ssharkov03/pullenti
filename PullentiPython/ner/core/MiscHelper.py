...
from PullentiPython.ner.core.internal.RusLatAccord import RusLatAccord
...
from PullentiPython.ner.core.CanBeEqualsAttr import CanBeEqualsAttr
...
class MiscHelper:
    ...
    def can_be_equals_ex(s1 : str, s2 : str, attrs : 'CanBeEqualsAttr') -> bool:
        ...
        from PullentiPython.ner.core.AnalysisKit import AnalysisKit
    ...
    def get_text_value_of_meta_token(mt : 'MetaToken', attrs : 'GetTextAttr'=GetTextAttr.NO) -> str:
        ...
        """
        from PullentiPython.ner.core.NounPhraseMultivarToken import NounPhraseMultivarToken
        if (mt is None): 
            return None
                            ...
                            if (it.gender == npt.morph.gender or ((it.gender) & (npt.morph.gender)) != (MorphGender.UNDEFINED)): 
                                if (not ((it.case_) & npt.morph.case_).is_undefined): 
                                    if (it.number == npt.morph.number or ((it.number) & (npt.morph.number)) != (MorphNumber.UNDEFINED)): 
                                        ...
                                        if (isinstance(it, MorphWordForm)): 
                                            var = it.normal_case
                                        bi = MorphBaseInfo._new676(MorphClass.ADJECTIVE, npt.morph.gender, npt.morph.number, npt.morph.language)
                ...
                if (isinstance(t, NumberToken)): 
                    ...
                    if (not t.chars.is_letter): 
                        ...
                        if ((t.next0_ is not None and t.next0_.is_hiphen and t.is_value("ГЕНЕРАЛ", None)) or t.is_value("КАПИТАН", None)): 
                            ...
                            mbi = MorphBaseInfo._new677(npt.morph.gender, cas, MorphNumber.SINGULAR)
                            if (num != MorphNumber.UNDEFINED): 
                                mbi.number = num
    ...
    def get_text_morph_var_by_case_and_number_ex(str0_ : str, cas : 'MorphCase'=None, num : 'MorphNumber'=MorphNumber.SINGULAR, num_val : str=None) -> str:
        ...
        if (str0_ == "лет"): 
            str0_ = "год"
        ar = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new678(str0_, False), None, None)
        if (ar is None or ar.first_token is None): 
            return str0_
        ...
        adj_bi = MorphBaseInfo._new679((MorphClass.ADJECTIVE) | MorphClass.NOUN, cas, num, npt.morph.gender)
        noun_bi = MorphBaseInfo._new680(npt.noun.morph.class0_, cas, num)
        ...
        if (not Utils.isNullOrEmpty(num_val) and num == MorphNumber.UNDEFINED): 
            ...
            Utils.tryParseInt(num_val, wrapn681)
            n = wrapn681.value
