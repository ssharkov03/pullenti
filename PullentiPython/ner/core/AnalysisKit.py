...
from PullentiPython.unisharp.Misc import EventHandler
...
from PullentiPython.ner.core.StatisticCollection import StatisticCollection
...
from PullentiPython.ner.core.internal.GeneralRelationHelper import GeneralRelationHelper
...
class AnalysisKit:
        ...
        if (tokens is not None): 
            ...
            while ii < len(tokens): 
                ...
                if (sofa_.correction_dict is not None): 
                    corw = None
                    wrapcorw655 = RefOutArgWrapper(None)
                    inoutres656 = Utils.tryGetValue(sofa_.correction_dict, mt.term, wrapcorw655)
                    corw = wrapcorw655.value
                    if (inoutres656): 
                        ccc = MorphologyService.process(corw, lang, None)
                        if (ccc is not None and len(ccc) == 1): 
                            tt1 = TextToken._new654(ccc[0], self, tt.begin_char, tt.end_char, tt.term)
                            tt1.chars = tt.chars
                            tt = tt1
    ...
    def correct_word_by_morph(self, tt : 'Token', lang : 'MorphLang') -> 'Token':
        ...
        if (ccc is None or len(ccc) != 1): 
            return None
        tt1 = TextToken._new657(ccc[0], self, tt.begin_char, tt.end_char, tt.chars, tt.term)
    ...
    def embed_token(self, mt : 'MetaToken') -> None:
        ...
        if (isinstance(mt, ReferentToken)): 
            if (mt.referent is not None): 
                mt.referent.add_occurence(TextAnnotation._new658(self.sofa, mt.begin_char, mt.end_char))
    ...
    def get_analyzer_data(self, analyzer : 'Analyzer') -> 'AnalyzerData':
        ...
        if (analyzer is None or analyzer.name is None): 
            return None
        d = None
        wrapd659 = RefOutArgWrapper(None)
        inoutres660 = Utils.tryGetValue(self.__m_datas, analyzer.name, wrapd659)
        d = wrapd659.value
        if (inoutres660): 
            d.kit = self
            return d
    ...
    def deserialize(self, stream : Stream) -> bool:
        ...
        self.base_language = MorphLang._new215(SerializerHelper.deserialize_int(stream))
        self.__m_entities = list()
    ...
    @staticmethod
    def _new2615(_arg1 : 'Processor', _arg2 : 'ExtOntology') -> 'AnalysisKit':
        res = AnalysisKit()
        res.processor = _arg1
        res.ontology = _arg2
        return res
    ...
    @staticmethod
    def _new2616(_arg1 : 'SourceOfAnalysis', _arg2 : bool, _arg3 : 'MorphLang', _arg4 : EventHandler, _arg5 : 'ExtOntology', _arg6 : 'Processor', _arg7 : bool) -> 'AnalysisKit':
