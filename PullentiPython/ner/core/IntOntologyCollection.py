...
from PullentiPython.ner.core.IntOntologyToken import IntOntologyToken
...
class IntOntologyCollection:
    ...
    class OntologyTermin(Termin):
        ...
        @staticmethod
        def _new673(_arg1 : 'IntOntologyItem', _arg2 : object) -> 'OntologyTermin':
    ...
    def add_item(self, di : 'IntOntologyItem') -> None:
        ...
        while i < len(di.termins): 
            if (isinstance(di.termins[i], IntOntologyCollection.OntologyTermin)): 
                ...
                nt = IntOntologyCollection.OntologyTermin._new673(di, di.termins[i].tag)
    ...
    def add_termin(self, di : 'IntOntologyItem', t : 'Termin') -> None:
        nt = IntOntologyCollection.OntologyTermin._new673(di, t.tag)
        t.copy_to(nt)
        self.__m_termins.add(nt)
        ...
        for tt in tts: 
            ...
            if (di is not None): 
                ...
                if (di in dis): 
                    continue
                dis.append(di)
            res.append(IntOntologyToken._new675(tt.begin_token, tt.end_token, di, tt.termin, tt.morph))
    ...
    @staticmethod
    def _new2591(_arg1 : bool) -> 'IntOntologyCollection':
