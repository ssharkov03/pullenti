...
from PullentiPython.ner.core.internal.NounPhraseItemTextVar import NounPhraseItemTextVar
...
from PullentiPython.ner.core.PrepositionHelper import PrepositionHelper
...
from PullentiPython.ner.core.internal.NounPhraseItem import NounPhraseItem
from PullentiPython.ner.core.VerbPhraseHelper import VerbPhraseHelper
...
class _NounPraseHelperInt:
    ...
    def __try_parse_ru(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int, def_noun : 'NounPhraseItem'=None) -> 'NounPhraseToken':
        ...
        if (noun is None): 
            return None
        res = NounPhraseToken._new650(first, noun.end_token, prep)
        if (adverbs is not None): 
            for a in adverbs: 
        ...
        if (stat is not None): 
            for adj in items: 
                if (adj.morph.items_count > 1): 
                    ...
                    Utils.tryGetValue(stat, l1, wrapi1652)
                    i1 = wrapi1652.value
                    wrapi2651 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l2, wrapi2651)
                    i2 = wrapi2651.value
                    if (i1 < i2): 
    ...
    def __try_parse_en(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int) -> 'NounPhraseToken':
        ...
        if (res.morph.items_count == 0 and has_article): 
            res.morph.add_item(MorphBaseInfo._new332(MorphClass.NOUN, MorphNumber.SINGULAR))
        i = 0
        while i < (len(items) - 1): 
            res.adjectives.append(items[i])
            i += 1
