...
from PullentiPython.ner.core.TableCellToken import TableCellToken
from PullentiPython.ner.core.TableRowToken import TableRowToken
...
class TableHelper:
    ...
    def try_parse_rows(t : 'Token', max_char : int, must_be_start_of_table : bool) -> typing.List['TableRowToken']:
        ...
        if (must_be_start_of_table): 
            if (not t.is_char(chr(0x1E))): 
                return None
            is_tab = True
        wrapis_tab750 = RefOutArgWrapper(is_tab)
        rw = TableHelper.__parse(t, max_char, None, wrapis_tab750)
        is_tab = wrapis_tab750.value
        if (rw is None): 
            return None
        res = list()
        res.append(rw)
        ...
        while t is not None: 
            wrapis_tab749 = RefOutArgWrapper(is_tab)
            rw0 = TableHelper.__parse(t, max_char, rw, wrapis_tab749)
            is_tab = wrapis_tab749.value
            if (rw0 is None): 
                break
            rw = rw0
            res.append(rw)
    ...
    def __parse(t : 'Token', max_char : int, prev : 'TableRowToken', is_tab : bool) -> 'TableRowToken':
        ...
        res.cells.append(TableCellToken._new751(t, tt, cell_info.row_span, cell_info.col_span))
        tt = tt.next0_
        while tt is not None and ((tt.end_char <= max_char or max_char == 0)): 
            t0 = tt
            cell_info = (None)
            ...
            if (cell_info.typ == TableHelper.TableTypes.ROWEND): 
                if (tt != t0): 
                    res.cells.append(TableCellToken._new751(t0, tt, cell_info.row_span, cell_info.col_span))
                res.end_token = tt
                res._eor = True
                break
            if (cell_info.typ != TableHelper.TableTypes.CELLEND): 
