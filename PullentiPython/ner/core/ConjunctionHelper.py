...
from PullentiPython.ner.core.ConjunctionToken import ConjunctionToken
...
from PullentiPython.ner.core.ConjunctionType import ConjunctionType
...
class ConjunctionHelper:
    ...
    def try_parse(t : 'Token') -> 'ConjunctionToken':
        ...
        if (t.is_comma): 
            ...
            if (ne is not None): 
                ...
                ne.is_simple = False
                return ne
            return ConjunctionToken._new662(t, t, ConjunctionType.COMMA, True, ",")
            ...
            if (tok.termin.tag2 is not None): 
                ...
                if (tok.end_token.get_morph_class_in_dictionary().is_verb): 
                    if (not tok.end_token.term.endswith("АЯ")): 
                        return None
            return ConjunctionToken._new663(t, tok.end_token, tok.termin.canonic_text, Utils.valToEnum(tok.termin.tag, ConjunctionType))
        if (not t.get_morph_class_in_dictionary().is_conjunction): 
            return None
        if (t.is_and or t.is_or): 
            res = ConjunctionToken._new664(t, t, t.term, True, (ConjunctionType.OR if t.is_or else ConjunctionType.AND))
        ...
        term = t.term
        if (term == "НИ"): 
            return ConjunctionToken._new663(t, t, term, ConjunctionType.NOT)
        if ((term == "А" or term == "НО" or term == "ЗАТО") or term == "ОДНАКО"): 
            return ConjunctionToken._new663(t, t, term, ConjunctionType.BUT)
        return None
    ...
    def _initialize() -> None:
        ...
        te = None
        te = Termin._new264("ТАКЖЕ", ConjunctionType.AND)
        ...
        te = Termin._new264("ЕСЛИ", ConjunctionType.IF)
        ConjunctionHelper.__m_ontology.add(te)
        te = Termin._new264("ТО", ConjunctionType.THEN)
        ConjunctionHelper.__m_ontology.add(te)
        te = Termin._new264("ИНАЧЕ", ConjunctionType.ELSE)
        ConjunctionHelper.__m_ontology.add(te)
        te = Termin._new266("ИНАЧЕ КАК", ConjunctionType.EXCEPT, True)
        ...
        te.add_abridge("КРОМЕ, КАК")
        ConjunctionHelper.__m_ontology.add(te)
        te = Termin._new266("ВКЛЮЧАЯ", ConjunctionType.INCLUDE, True)
