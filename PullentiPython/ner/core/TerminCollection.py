...
class TerminCollection:
    ...
    def __add_to_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                nod.children = dict()
            nn = None
            wrapnn770 = RefOutArgWrapper(None)
            inoutres771 = Utils.tryGetValue(nod.children, ch, wrapnn770)
            nn = wrapnn770.value
            if (not inoutres771): 
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                ...
                Utils.tryGetValue(nod.children, ch, wrapnn774)
                nn = wrapnn774.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli775 = RefOutArgWrapper(None)
        inoutres776 = Utils.tryGetValue(self.__m_hash1, key, wrapli775)
        li = wrapli775.value
        if (not inoutres776): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
            ...
            if ((((pars) & (TerminParseAttr.TERMONLY))) != (TerminParseAttr.NO)): 
                ...
                while i < tt.invariant_prefix_length_of_morph_vars: 
                    ...
                    if (nod.children is None): 
                        ...
                        break
                    nn = None
                    wrapnn777 = RefOutArgWrapper(None)
                    inoutres778 = Utils.tryGetValue(nod.children, ch, wrapnn777)
                    nn = wrapnn777.value
                    if (not inoutres778): 
                        no_vars = True
                        break
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn779 = RefOutArgWrapper(None)
                        inoutres780 = Utils.tryGetValue(nod.children, ch, wrapnn779)
                        nn = wrapnn779.value
                        if (not inoutres780): 
                            no_vars = True
                            break
                        ...
                        i += 1
            if (not no_vars): 
                wrapres785 = RefOutArgWrapper(res)
                inoutres786 = self.__manage_var(token, pars, s, nod, len0, wrapres785)
                res = wrapres785.value
                if (inoutres786): 
                    was_vars = True
                i = 0
                first_pass2835 = True
                while True:
                    ...
                    if (wf.normal_case is None or wf.normal_case == s): 
                        ...
                        if (j < i): 
                            ok = False
                    if (ok): 
                        wrapres781 = RefOutArgWrapper(res)
                        inoutres782 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres781)
                        res = wrapres781.value
                        if (inoutres782): 
                            was_vars = True
                    ...
                    if (j < i): 
                        continue
                    wrapres783 = RefOutArgWrapper(res)
                    inoutres784 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres783)
                    res = wrapres783.value
                    if (inoutres784): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres787 = RefOutArgWrapper(res)
            inoutres788 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres787)
            res = wrapres787.value
            if (inoutres788): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars789 = RefOutArgWrapper(None)
            inoutres790 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars789)
            vars0_ = wrapvars789.value
            if (inoutres790): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn791 = RefOutArgWrapper(None)
            inoutres792 = Utils.tryGetValue(nod.children, ch, wrapnn791)
            nn = wrapnn791.value
            if (not inoutres792): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ct = t.canonic_text
                li = [ ]
                wrapli793 = RefOutArgWrapper(None)
                inoutres794 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli793)
                li = wrapli793.value
                if (not inoutres794): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres795 = RefOutArgWrapper(None)
        inoutres796 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres795)
        res = wrapres795.value
        if (not inoutres796): 
            return None
        else: 
            return res