...
from PullentiPython.morph.MorphTense import MorphTense
from PullentiPython.morph.MorphVoice import MorphVoice
from PullentiPython.morph.MorphBaseInfo import MorphBaseInfo
from PullentiPython.morph.MorphMood import MorphMood
from PullentiPython.morph.MorphPerson import MorphPerson
from PullentiPython.morph.internal.ByteArrayWrapper import ByteArrayWrapper
from PullentiPython.morph.internal.MorphDeserializer import MorphDeserializer
from PullentiPython.morph.internal.MorphRule import MorphRule
from PullentiPython.morph.internal.MorphTreeNode import MorphTreeNode
from PullentiPython.morph.MorphLang import MorphLang
from PullentiPython.morph.MorphMiscInfo import MorphMiscInfo
...
class MorphEngine:
    ...
    def initialize(self, lang : 'MorphLang', lazy_load : bool) -> bool:
        ...
        with self._m_lock: 
            ...
            names = Utils.getResourcesNames('PullentiPython.morph.internal.properties', '.dat')
            for n in names: 
                if (Utils.endsWithString(n, rsname, True)): 
                    inf = Utils.getResourceInfo('PullentiPython.morph.internal.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.morph.internal.properties', n) as stream: 
                        stream.position = 0
    ...
    def __load_tree_node(self, tn : 'MorphTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos167 = RefOutArgWrapper(pos)
                tn._deserialize_lazy(self.__m_lazy_buf, self, wrappos167)
                pos = wrappos167.value
            tn.lazy_pos = 0
    ...
    def process(self, word : str, ignore_no_dict : bool=False) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn168 = RefOutArgWrapper(None)
            inoutres169 = Utils.tryGetValue(tn.nodes, ch, wraptn168)
            tn = wraptn168.value
            if (not inoutres169): 
                break
            i += 1
    ...
    def get_all_wordforms(self, word : str) -> typing.List['MorphWordForm']:
        ...
        while i <= len(word): 
            ...
            if (tn.nodes is None or i >= len(word)): 
                break
            ch = ord(word[i])
            wraptn170 = RefOutArgWrapper(None)
            inoutres171 = Utils.tryGetValue(tn.nodes, ch, wraptn170)
            tn = wraptn170.value
            if (not inoutres171): 
                break
            i += 1
    ...
    def get_wordform(self, word : str, cla : 'MorphClass', gender : 'MorphGender', cas : 'MorphCase', num : 'MorphNumber', add_info : 'MorphWordForm') -> str:
        ...
        while i <= len(word): 
            ...
            inoutres173 = Utils.tryGetValue(tn.nodes, ch, wraptn172)
            tn = wraptn172.value
            if (not inoutres173): 
                break
            i += 1
    ...
    def deserialize(self, str0 : Stream, ignore_rev_tree : bool, lazy_load : bool) -> None:
        ...
        pos = 0
        wrappos182 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos182)
        pos = wrappos182.value
        while cou > 0: 
            mi = MorphMiscInfo()
            wrappos174 = RefOutArgWrapper(pos)
            mi._deserialize(buf, wrappos174)
            pos = wrappos174.value
            self.add_misc_info(mi)
            cou -= 1
        wrappos181 = RefOutArgWrapper(pos)
        cou = buf.deserialize_int(wrappos181)
        pos = wrappos181.value
        while cou > 0: 
            wrappos176 = RefOutArgWrapper(pos)
            p1 = buf.deserialize_int(wrappos176)
            pos = wrappos176.value
            r = MorphRule()
            if (lazy_load): 
                r.lazy_pos = pos
                pos = p1
            else: 
                wrappos175 = RefOutArgWrapper(pos)
                r._deserialize(buf, wrappos175)
                pos = wrappos175.value
            self.add_rule(r)
            cou -= 1
        root = MorphTreeNode()
        if (lazy_load): 
            wrappos177 = RefOutArgWrapper(pos)
            root._deserialize_lazy(buf, self, wrappos177)
            pos = wrappos177.value
        else: 
            wrappos178 = RefOutArgWrapper(pos)
            root._deserialize(buf, wrappos178)
            pos = wrappos178.value
        self.m_root = root
        if (not ignore_rev_tree): 
            root_rev = MorphTreeNode()
            if (lazy_load): 
                wrappos179 = RefOutArgWrapper(pos)
                root_rev._deserialize_lazy(buf, self, wrappos179)
                pos = wrappos179.value
            else: 
                wrappos180 = RefOutArgWrapper(pos)
                root_rev._deserialize(buf, wrappos180)
                pos = wrappos180.value
