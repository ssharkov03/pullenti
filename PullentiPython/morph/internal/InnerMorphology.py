...
from PullentiPython.unisharp.Misc import ProgressEventArgs

from PullentiPython.morph.CharsInfo import CharsInfo
from PullentiPython.morph.MorphToken import MorphToken
from PullentiPython.morph.internal.UnicodeInfo import UnicodeInfo
from PullentiPython.morph.MorphClass import MorphClass
from PullentiPython.morph.internal.MorphEngine import MorphEngine
from PullentiPython.morph.MorphLang import MorphLang
from PullentiPython.morph.MorphWordForm import MorphWordForm
from PullentiPython.morph.internal.TextWrapper import TextWrapper
from PullentiPython.morph.internal.UniLexWrap import UniLexWrap
...
class InnerMorphology:
    ...
    def run(self, text : str, only_tokenizing : bool, dlang : 'MorphLang', good_text : bool, progress : EventHandler) -> typing.List['MorphToken']:
        ...
        while True:
            ...
            if (ty == 1 and not only_tokenizing): 
                wraplemmas160 = RefOutArgWrapper(None)
                inoutres161 = Utils.tryGetValue(uni_lex, term, wraplemmas160)
                lemmas = wraplemmas160.value
                if (not inoutres161): 
        ...
        if (((def_lang.is_undefined or def_lang.is_ua)) and tot_rus_words > 0): 
            if (((tot_ukr_words > tot_rus_words and self.__m_engine_ua.language.is_ua)) or ((tot_by_words > tot_rus_words and self.__m_engine_by.language.is_by)) or ((tot_kz_words > tot_rus_words and self.__m_engine_kz.language.is_kz))): 
                ...
                for kp in uni_lex.items(): 
                    lang = MorphLang()
                    wraplang162 = RefOutArgWrapper(lang)
                    kp[1].word_forms = self.__process_one_word(kp[0], wraplang162)
                    lang = wraplang162.value
        ...
        for kp in uni_lex.items(): 
            ...
            if (lang.is_undefined): 
                if (tot_rus_words > tot_by_words and tot_rus_words > tot_ukr_words and tot_rus_words > tot_kz_words): 
                    ...
                    lang = MorphLang.KZ
            wraplang163 = RefOutArgWrapper(lang)
            kp[1].word_forms = self.__process_one_word(kp[0], wraplang163)
            lang = wraplang163.value
            kp[1].lang = lang
            if (lang.is_ru): 
                tot_rus_words += 1
            if (lang.is_ua): 
            ...
            if (mt.char_info.is_last_lower and mt.length > 2 and mt.char_info.is_cyrillic_letter): 
                ...
                for wf in mt.word_forms: 
                    if (wf.normal_case == pref or wf.normal_full == pref): 
                        ...
                        break
                if (not ok): 
                    wf0 = MorphWordForm._new164(pref, MorphClass.NOUN, 1)
    ...
    def __process_one_word0(self, wstr : str) -> typing.List['MorphWordForm']:
        dl = MorphLang()
        wrapdl165 = RefOutArgWrapper(dl)
        inoutres166 = self.__process_one_word(wstr, wrapdl165)
        dl = wrapdl165.value
        return inoutres166
