...
from PullentiPython.morph.MorphFinite import MorphFinite
from PullentiPython.morph.MorphAspect import MorphAspect
...
class LanguageHelper:
    ...
    def get_case_after_preposition(prep : str) -> 'MorphCase':
        mc = None
        wrapmc184 = RefOutArgWrapper(None)
        inoutres185 = Utils.tryGetValue(LanguageHelper.__m_prep_cases, prep, wrapmc184)
        mc = wrapmc184.value
        if (inoutres185): 
            return mc
        else: 
    ...
    def normalize_preposition(prep : str) -> str:
        res = None
        wrapres186 = RefOutArgWrapper(None)
        inoutres187 = Utils.tryGetValue(LanguageHelper.__m_prep_norms, prep, wrapres186)
