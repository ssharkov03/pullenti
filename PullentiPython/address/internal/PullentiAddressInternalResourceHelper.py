...
class PullentiAddressInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('PullentiPython.address.properties', '.png')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('PullentiPython.address.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('PullentiPython.address.properties', n) as stream: 
