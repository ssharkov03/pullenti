...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new579(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new579(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi582 = RefOutArgWrapper(None)
                inoutres583 = Utils.tryGetValue(all0_, a.name, wrapsi582)
                si = wrapsi582.value
                if (not inoutres583): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli580 = RefOutArgWrapper(None)
                    inoutres581 = Utils.tryGetValue(si, s, wrapli580)
                    li = wrapli580.value
                    if (not inoutres581): 
                        li = list()
                        si[s] = li
