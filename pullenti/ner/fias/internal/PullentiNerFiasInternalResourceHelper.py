...
class PullentiNerFiasInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('pullenti.address.internal.gar.properties', '.png')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('pullenti.address.internal.gar.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('pullenti.address.internal.gar.properties', n) as stream: 
